...
def _serialize_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Release: {_markdown_escape(system.get('release_version', 'unavailable'))}",
        f"- Web commit: {_markdown_escape(system.get('components', {}).get('web', {}).get('commit_sha', 'unavailable'))}",
        f"- API commit: {_markdown_escape(system.get('components', {}).get('api', {}).get('commit_sha', 'unavailable'))}",
        f"- Worker commit: {_markdown_escape(system.get('components', {}).get('worker', {}).get('commit_sha', 'unavailable'))}",
        f"- DB schema: {_markdown_escape(system.get('schema_revision', 'unavailable'))}",
        "",
        "## Environment summary",
        f"- Environment: {_markdown_escape(system['environment'])}",
        f"- Backend: {_markdown_escape(system.get('health', {}).get('backend', 'unavailable'))}",
        f"- Queue: {_markdown_escape(system.get('health', {}).get('queue', {}).get('status', 'unavailable'))}",
        f"- Worker: {_markdown_escape(system.get('health', {}).get('worker', {}).get('status', 'unavailable'))}",
        f"- Object storage: {_markdown_escape(system.get('health', {}).get('object_storage', {}).get('status', 'unavailable'))}",
        f"- STT provider: {_markdown_escape(system.get('health', {}).get('stt_provider', {}).get('status', 'unavailable'))}",
