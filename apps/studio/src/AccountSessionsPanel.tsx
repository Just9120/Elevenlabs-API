  ...
  const reconcileTarget = async (sessionId: string) => {
    ...
    setMessage(
      confirmed
        ...
        : observed
          ? "Сервер не подтвердил завершение сессии. Список обновлён; проверьте его перед повтором."
          : "Сервер не подтвердил завершение сессии, а обновить список не удалось. Обновите страницу перед повтором.",
    );
  };
  ...
  const reconcileOthers = async () => {
    ...
    setMessage(
      confirmed
        ...
        : observed
          ? "Сервер не подтвердил завершение всех остальных сессий. Список обновлён; проверьте его перед повтором."
          : "Сервер не подтвердил завершение остальных сессий, а обновить список не удалось. Обновите страницу перед повтором.",
    );
  };
