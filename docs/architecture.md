...
| --- | --- | --- | --- |
| Studio frontend | `apps/studio/` | Browser UI for sessions, user-facing ordinary/Live `Транскрибации`, internal legacy workspaces, sources, credentials, primary Google/Picker connection, separate maintenance consent, preparation, grouped multi-transcriptions with item-level jobs/outputs, manual speaker profile/listen/assign flow, diagnostics, standardization, `Манифест Studio`, and Live workspace with IndexedDB/server draft recovery; `src/apiClient.ts` owns same-origin JSON/CSRF retry transport and its safe diagnostic emission. | Current merge/CI/deployment evidence belongs in `docs/delivery-plan.md`; source presence is not live Google, recovery or Live-capture evidence. |
...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/jobs/outputs/diagnostics/catalog state, owner-scoped output-folder Favorites, authoritative optional source-creation metadata, separately encrypted maintenance grant fields, bounded provider-part progress counters, immutable optional media-clip bounds, safe underlying provider failure categories, TTL-bounded encrypted completed-part checkpoints, and 72-hour encrypted Live transcript drafts. | Branch migrations are present through `0023_realtime_transcript_drafts`; exact production revision evidence is tracked separately in `docs/delivery-plan.md`. |
