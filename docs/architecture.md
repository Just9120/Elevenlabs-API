...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/jobs/outputs/diagnostics/catalog state, separately encrypted maintenance grant fields, bounded provider-part progress counters, immutable optional media-clip bounds, safe underlying provider failure categories, and TTL-bounded encrypted completed-part checkpoints. | Branch migrations are present through `0020_partial_provider_checkpoints`; exact production revision evidence is tracked separately in `docs/delivery-plan.md`. |
| Alembic migrations | `apps/studio-api/alembic/versions/` | Schema authority for Studio persistence. | Current branch head is `0020_partial_provider_checkpoints`; ordinary component CD does not apply it. The separately enabled protected release lane may apply only one reviewed direct additive successor per approval across the `0018` then `0019` then `0020` chain. |
