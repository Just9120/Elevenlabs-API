...
The authority remains partial by design: the separately initiated one-time migration can discover, standardize, and import approved legacy evidence, but a document without an explicit source identity cannot participate in duplicate matching and indeterminate settings require an explicit reprocess decision. The authority does not infer linkage from a document name, detect a separately uploaded copy of the same local file, treat queued/processing jobs as accepted outputs, or turn the one-time migration into continuous Drive synchronization. Reuse/skip needs an explicit accepted-output linkage design and is not inferred from a match.
