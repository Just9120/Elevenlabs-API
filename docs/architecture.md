...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/jobs/outputs/diagnostics/catalog state, owner-scoped output-folder Favorites, authoritative optional source-creation metadata, separately encrypted maintenance grant fields, bounded provider-part progress counters, immutable optional media-clip bounds, safe underlying provider failure categories, and TTL-bounded encrypted completed-part checkpoints. | Branch migrations are present through `0021_source_creation_favorites`; exact production revision evidence is tracked separately in `docs/delivery-plan.md`. |
