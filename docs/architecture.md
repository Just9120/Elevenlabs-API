...
6. Stop sends one final commit signal, allows a bounded final-event grace period, then closes capture/socket/audio resources. Unsupported capture APIs, permission rejection, source-ended, capability timeout/abort, socket/provider error, backpressure overflow, a 10-second connection timeout, page hide, and unmount also clean up deterministically.
