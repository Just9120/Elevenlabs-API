...
The first catalog foundation is deliberately server-only and does not yet change that preflight result. It defines one canonical current settings tuple (`elevenlabs`, `scribe_v2`, `ru|detect`, diarization boolean), reuses `transcript_doc_v1.2` and the accepted Google Docs output kind as shared constants, and can classify existing persisted Studio output evidence as an accepted exact match, standardization-required, indeterminate, or no match. Classification is owner-scoped across projects: a Google source uses its private Drive file ID so reselecting the same file can retain identity, while a local upload uses its existing Studio source row because no content fingerprint is currently persisted. These private identities and output records remain server-only.

This foundation reads existing PostgreSQL authority without a schema migration, Google call, document-body read, provider call, skip, reprocess override, or user-visible conflict decision. It is partial by design: it does not discover the historical Drive collection and cannot detect a separately uploaded copy of the same local file. Preflight/create integration must continue to report catalog authority as unavailable until the explicit conflict flow and atomic create-time recheck are implemented; the later one-time migration must add imported evidence without turning this path into continuous Drive synchronization.
