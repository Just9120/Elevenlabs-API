...
| Alembic migrations | `apps/studio-api/alembic/versions/` | Schema authority for Studio persistence. | Current branch head is additive direct successor `0021_source_creation_favorites`; ordinary component CD does not apply it. The separately enabled protected release lane may apply only one reviewed direct additive successor per approval. |
