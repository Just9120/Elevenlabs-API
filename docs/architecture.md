...
2. The browser obtains explicit microphone, display/tab-audio, or both permissions. A mixed session combines sources locally with voice-priority gain staging: the microphone remains at unity gain while display audio is attenuated before the combined PCM path so loud playback does not mask near-field speech. The microphone requests standard browser echo cancellation, noise suppression, automatic gain control and mono capture as ideal constraints; headphones remain the recommended acoustic boundary. No captured bytes transit Studio API, PostgreSQL, Redis, object storage, or the worker.
