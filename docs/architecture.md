...
| Source expiry authority | Stores the pending-upload deadline at initiation and replaces it with the retained-source deadline only after exact object metadata verification. | PostgreSQL `sources.expires_at` is authoritative; the PWA only displays the exact server value. |
