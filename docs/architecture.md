...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/jobs/outputs/diagnostics/catalog state, separately encrypted maintenance grant fields, and bounded provider-part progress counters. | Branch migrations are present through `0018_job_part_progress`; production revision evidence is tracked separately in `docs/delivery-plan.md`. |
| Alembic migrations | `apps/studio-api/alembic/versions/` | Schema authority for Studio persistence. | Current branch head is `0018_job_part_progress`; ordinary component CD does not apply it. A separately enabled, protected release lane may apply exactly one reviewed direct additive successor. |
