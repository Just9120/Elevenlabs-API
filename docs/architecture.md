...
Studio PWA is a web platform contour in development. Source-level architecture includes the web frontend, API, PostgreSQL, Redis, object storage, worker entrypoint, provider adapter path, Google Drive/Docs integration, diagnostics, independent recursive transcript-maintenance operations, and migrations. The bounded single-worker/small-source processing path has controlled production evidence, while broader selected capabilities and every newer schema/API revision still require their own rollout evidence.
...
| Google integration | Google OAuth/Drive/Docs modules under `apps/studio-api/studio_api/` | Narrow primary `drive.file` Picker capability, separately encrypted server-only maintenance grant, safe Drive traversal, Google Docs reads/writes, and output creation. | Primary Picker/output evidence does not prove the separate maintenance grant or recursive operation behavior. |
| Transcript maintenance | `transcript_catalog*.py`, `transcript_maintenance*.py`, migrations `0016`/`0017`, and frontend maintenance panel/model modules | Two owner-scoped operations over independently selected recursive roots: in-place standardization or PostgreSQL-only `Манифест Studio` import, with fresh scans and explicit conflict outcomes. | Source and targeted-test state is tracked in `docs/delivery-plan.md`; migration/config/deployment/live dry-run/apply evidence remains separate. |
...
Recursive transcript maintenance starts through a second OAuth consent but remains a server-only integration after the authorization redirect. The API stores its encrypted refresh token in separate columns, requires the same Google subject as the primary connection, and accepts only identity plus `drive.metadata.readonly` and `documents`. The browser receives only safe maintenance connection state and operation results. It selects one root folder through the primary Picker and sends only that `folder_id`; it never supplies document IDs or a saved candidate set.
...
Transcript maintenance is not part of job creation. Standardization and `Манифест Studio` each own a separate selected root, preview, confirmation, and result. For dry-run and again for apply, the API traverses the root and descendants under explicit page/item/folder bounds, processes only native Google Docs, and rejects cycles, malformed listings, duplicate identities, repeated page tokens, incomplete search, or exceeded limits. Standardization skips current documents and may rewrite only eligible non-current documents in place. `Манифест Studio` skips already-target entries and may persist only eligible current-document metadata. Per-document unsafe/inaccessible outcomes do not abort safe siblings; connection-wide auth, rate-limit, availability, timeout, or global scan failures abort.
