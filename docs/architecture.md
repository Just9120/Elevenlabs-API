...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/jobs/outputs/diagnostics/catalog state, separately encrypted maintenance grant fields, bounded provider-part progress counters, and immutable optional media-clip bounds on each job. | Branch migrations are present through `0019_job_media_clip`; production revision evidence is tracked separately in `docs/delivery-plan.md`. |
| Alembic migrations | `apps/studio-api/alembic/versions/` | Schema authority for Studio persistence. | Current branch head is `0019_job_media_clip`; ordinary component CD does not apply it. A separately enabled, protected release lane may apply the reviewed direct additive `0018` then `0019` chain. |
