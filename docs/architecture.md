...
`/audio` is an independent pre-transcription workspace and appears immediately before `/transcriptions` in navigation. It exposes two explicit processing boundaries. In the browser-local path the selected device bytes never leave the tab: Web Audio decodes supported codecs under file-count/input-byte/decoded-sample bounds, applies ordered separate/concat, channel and silence transforms, encodes PCM16 WAV, and keeps object-URL results only until reload/close unless the owner explicitly downloads or uploads one result. Unsupported codecs/resources fail with a server-side Studio fallback. In the durable Studio path, owner-scoped existing Drive/local Sources or newly uploaded ephemeral local references become ordered `AudioPreparationJobInput` rows. The worker performs bounded FFprobe/full-decode validation, computes a non-mutating duration preview, then executes an allowlisted shell-free FFmpeg plan for compatible copy-concat or WAV/FLAC conversion, mono channel selection and bounded silence removal. PostgreSQL owns preview/processing/cancel/checkpoint/lease state; expired active leases are recoverable and a stage heartbeat prevents concurrent long-running ownership. Output is a deterministic owner/job-scoped S3 object and reusable Source under normal owner retention, with authenticated short-lived download, or an idempotently reconciled `drive.file` upload into an explicitly revalidated folder. Input references marked ephemeral are logically deleted at terminal state and have a 24-hour hard expiry; request-scoped media and partial output live only inside a temporary directory.
...
These responses require authenticated same-origin CSRF-protected issuance, `Cache-Control: no-store`, no service-worker runtime caching, no browser persistence/diagnostic logging, and server-side revalidation of every selected Drive ID or completed object. Picker issuance requires the exact identity + `drive.file + drive.readonly` boundary and exact Picker origin; because this browser capability carries broad read authority, reconnect/consent is explicit and any extra scope fails closed. Upload issuance uses an opaque server-owned object key, exact content type, PUT-only operation and at most 900 seconds; the cross-origin PUT omits browser credentials, exposes at most the public Studio origin as referrer under the edge policy, and does not accept a redirected terminal response. Refresh tokens, ID tokens, object keys, source bytes, provider secrets, and maintenance access/refresh tokens never cross this boundary.
...
- OAuth codes, raw payloads, owners/permissions, source bytes, ordinary transcript/document bodies, object keys, private paths, and stack traces are not browser payloads. The three bounded capability responses above and the authenticated owner-scoped temporary Live recovery DTO are the only explicit content/integration exceptions.
