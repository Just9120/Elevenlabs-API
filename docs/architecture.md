...
| Database | PostgreSQL via Studio deployment | Durable users/preferences/projects/sources/credentials/transcription and audio-preparation jobs/outputs/diagnostics/catalog state, owner-scoped output-folder Favorites, authoritative optional source-creation metadata, separately encrypted maintenance grant fields, bounded provider-part progress counters, immutable optional media-clip bounds, safe underlying provider failure categories, TTL-bounded encrypted completed-part checkpoints, and 72-hour encrypted Live transcript drafts. | Branch migrations are present through `0025_audio_preparation`; exact production revision evidence is tracked separately in `docs/delivery-plan.md`. |
| Alembic migrations | `apps/studio-api/alembic/versions/` | Schema authority for Studio persistence. | Current branch head is additive direct successor `0025_audio_preparation`; ordinary component CD does not apply it. The separately enabled protected release lane may apply only one reviewed direct additive successor per approval. |
...
| Diagnostics | API/frontend diagnostic modules, `diagnostic_reports.py` and migrations `0010`/`0011` | Safe diagnostic event/debug-session foundation and one allowlisted report model serialized as Markdown, JSON, YAML or TOML. The primary UI produces one selected-format diagnostic bundle with optional bounded user problem context; technical event/component/internal-ID filters remain an advanced disclosure. | Source present; every format reuses the same owner filters, redaction boundary and `no-store` download contract. User context is normalized, response-only, explicitly untrusted supporting input and is not persisted as runtime evidence. |
