...
Current Studio non-goals/deferred boundaries remain: no polling worker process, no queue consumer, no public processing pipeline, no manifest mutation, no production processing pipeline, no automatic migration rollout, and no production-live claim without runtime/operator evidence. Studio source now contains all internal per-stage boundaries through safe output persistence and fenced completion, plus an internal synchronous orchestrator for one already-leased job, an internal one-shot explicit-job claim boundary, and an internal single-iteration claim-next boundary that may select one ready queued PostgreSQL job and invoke that orchestrator once. These boundaries are not automatically invoked and are not a public or production processing pipeline. PWA-WORKER-01-PREP defines the next dedicated polling worker runtime contract before implementation; it does not add the worker process. Newly persisted output references are not exposed through the browser API.
