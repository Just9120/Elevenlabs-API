...
- Studio must present existing-document standardization and catalog import as two distinct user-facing operations, not one combined migration action. Each operation has its own document selection, non-mutating dry-run, safe aggregate result, and explicit apply confirmation. A dry-run or confirmation for one operation must never authorize the other.
- Both operations preserve the current `drive.file` OAuth boundary. The user first selects one folder and then explicitly selects a server-bounded set of Google Docs within that folder through Picker. Studio must not infer authority over pre-existing child documents from folder selection, perform recursive discovery, or silently broaden OAuth scope.
- Before either dry-run or apply, the server must reject duplicate, malformed, over-limit, non-Google-Doc, inaccessible, or out-of-folder document IDs. Apply must rebuild current evidence from the same explicit selected-document set and must not trust a saved browser preview.
- Existing-document standardization may read selected document content transiently to classify and normalize it. Explicit standardization apply may rewrite only the same selected Google Docs in place to `transcript_doc_v1.2`, including readable paragraph normalization. It must not create or update Studio catalog entries, persist source/job state, or call a transcription provider or LLM. A safe owner-scoped audit event is allowed.
- Catalog import may read selected document content transiently to classify its standard and may persist only the durable metadata needed for the Studio catalog and duplicate decisions. Catalog-import apply must not create, rewrite, standardize, move, or delete any Google Doc. Conflicting or insufficient source/settings authority must remain blocked rather than inferred. A safe owner-scoped audit event is allowed.
...
4. Conflicting, unreadable, inaccessible, out-of-folder, duplicate, changed, or insufficient-authority documents fail closed with safe outcomes.
...
- `PWA-TRANSCRIPT-CATALOG-MIGRATION-01` — split by explicit product decision; the old combined standardize-and-import action is no longer the target contract.
- `PWA-TRANSCRIPT-STANDARDIZATION-01` — independent explicit-document dry-run/apply for in-place `transcript_doc_v1.2` normalization, with no catalog persistence. Current combined source must be separated before this item can be source-complete.
- `PWA-TRANSCRIPT-CATALOG-IMPORT-01` — independent explicit-document dry-run/apply for minimal source-linked catalog and duplicate-authority metadata, with no Google Doc mutation. Production schema/API prerequisites are verified, but the separated source and production dry-run/apply remain.
