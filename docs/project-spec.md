...
2. Production database migration head matches the intended repository head where required. The current branch head is the direct additive chain `0018_job_part_progress` then `0019_job_media_clip` then `0020_provider_part_checkpoints`. The last exact revision retained in this document is `0017_google_maintenance_oauth`; live feature screenshots are not a substitute for rediscovering the exact database revision before a protected one-successor release.
