...
Verified repository baseline: `main@cb481be1a46a4f1ac091a0de63c518d023f799da` (invoice compatibility PR `#268`) has exact-main CI and verified API/web delivery `33425500261`. Worker remains on healthy `8957e974` with confirmed isolation; schema `0031` is unchanged. Public build identity and API readiness passed. LIVE subscription revalidation succeeded at `2026-08-31T18:38:08Z`; separate workspace usage still fails format validation and the single authorized transcription remains not-run. Exact identifiers are in `docs/delivery-plan.md`.

Current operational Goal: `PWA-WORKER-USAGE-ACCOUNTING-01`, state `IN_PROGRESS` (RESUME after requested owner diagnostic). Prior invoice fix PR `#268`, exact-main CI and API/web deployment passed; LIVE subscription is current. Owner diagnostic at exact `cb481be1` confirms HTTP 200 with credits in `total_usage`/unit `credits`, while the parser required `credits_used`. The bounded compatibility fix supports both schemas, rejects ambiguous metrics, preserves strict units and adds synthetic regression tests without changing permissions, STT model, schema or worker runtime. Focused validation `83 passed`; full portable `1260 passed, 5 skipped`; exact-head CI/API deployment and LIVE recheck pending. No upload or canary submitted and no new complete product AC counted. Existing post-deploy metadata is included in this substantive fix, not a metadata-only PR. Goal авторизует `PWAWOR-02..03` и `USAGEC-01..06`: worker resource/privilege/network/DB-role isolation, exact provider-call accounting, immutable nominal tariff snapshot и official ElevenLabs subscription/workspace-usage reconciliation с explicit freshness. Commercial contour, пользовательский billing, realtime/storage costs, pricing-page scraping и unsupported credits-to-minutes/invoice allocation вне scope.
...
Evidence: `SPEC ✅ | CODE ✅ | TEST ✅ | CI ◐ | DEPLOY ◐ | LIVE ◐`. Prior API/web `cb481be1` delivered by `33425500261`; schema `0031` and worker `8957e974` unchanged. LIVE subscription is current. Owner diagnostic confirms HTTP 200 with `total_usage`/`credits` and no `credits_used`; the new compatibility fix passed local focused/portable suites but still needs exact-head CI/API deployment and revalidation. No provider values or raw rows copied to tests/docs. Billed-duration canary remains not-run; no complete AC is inferred from partial checks.
