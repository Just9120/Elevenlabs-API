...
Studio current-task progress UX must represent honest execution state. The UI must not invent a percentage from an elapsed-time timer. Product-approved named stages are `В очереди`, `Подготовка источника`, `Отправка в ElevenLabs`, `Создание документа Google Docs`, `Сохранение результата`, and `Готово`. A displayed stage must come from authoritative backend state or a safely persisted processing event. When the exact stage is unknown, the UI should show an indeterminate indicator plus elapsed waiting time instead of a fabricated percent. A queued task that has been claimed but has not started `processing` must not visually look like ordinary infinite waiting without explanation.
...
Diagnostic levels are `ERROR`, `WARNING`, `INFO`, and `DEBUG`. Normal product diagnostics use `ERROR`, `WARNING`, and `INFO`. `DEBUG` is opt-in, time-bounded, server-authoritative, stops automatically, must not become permanent always-on telemetry, and one session may last no more than 30 minutes. A DEBUG session is owner-scoped, explicit, stored durably with server-side start and expiry timestamps, and cannot be extended by event ingestion or browser-supplied expiry. Enabling `DEBUG` must show the user when it expires.

Future diagnostic events, including browser/PWA ingestion, must be strict and allowlisted. Browser ingestion may submit only closed PWA event codes and safe scalar metadata; the server forces owner, component, request id, retention, and expiry authority. Conceptual fields are: opaque event id, occurred-at timestamp, level, component (`web`, `api`, or `worker`), stable event code, correlation id, request id where relevant, opaque project scope where relevant, opaque job scope where relevant, bounded safe metadata, and occurrence count for deduplicated repeated events. The server must allowlist event codes and metadata keys. Diagnostics must not accept arbitrary frontend event names, free-form log messages, or arbitrary JSON metadata. Correlation and request identifiers must be opaque and must not contain user data, tokens, URLs, filenames, transcript content, or secrets.
