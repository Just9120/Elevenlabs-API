...
Current Goal local Evidence: full Studio Vitest `705 passed`; full ESLint и TypeScript/Vite/PWA production build passed; focused Python filename/processor/download regressions `32 passed`; portable Python `1363 passed, 5 skipped, 9 unchanged Windows/WSL shell cases deselected`; lightweight checks, Python compileall и diff check passed. Tests подтверждают пустое поле, per-source separate fallback, deterministic concat fallback, local WAV inheritance, Unicode server output и RFC 5987 download metadata; exact-head Linux CI pending.
