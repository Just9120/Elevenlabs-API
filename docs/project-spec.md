...
Current operational Goal: `PWA-WORKER-USAGE-ACCOUNTING-01`, state `IN_PROGRESS`. `USAGEC-01/02/03/04/06` have delivered and applicable LIVE Evidence. The remaining `USAGEC-05` is implemented locally on `codex/output-reconciliation-job-cost`: owner job DTO/UI now separates confirmed nominal duration/cost and immutable rate provenance from provider account actuals and fails closed for unavailable/inconsistent data. The same branch removes an invalid `FOR UPDATE` lock on read-only `projects` while preserving locks on mutable output-authority rows. Portable Python `1267 passed`, Studio `659 passed`, focused suites, lint/build and lightweight checks pass; hosted PostgreSQL/CI/deploy/LIVE remain pending. No new provider call or worker resume occurred. Goal авторизует `PWAWOR-02..03` и `USAGEC-01..06`; commercial contour, пользовательский billing, realtime/storage costs, pricing-page scraping и unsupported credits-to-minutes/invoice allocation remain outside scope.
...
Evidence: `SPEC ✅ | CODE ✅ | TEST ✅ | CI ✅ | DEPLOY ✅ | LIVE ◐`. После operator master-key ownership correction worker deployment `33388441193`/job `99476297938` прошёл на exact `8957e974`; status `33388535135` подтвердил running/healthy, 2 CPU, 4 GiB memory, 256 PID, read-only rootfs, no-new-privileges, две разрешённые сети и exact bootstrap capability allowlist. Required worker health проверяет restricted DB role/grants и current runtime heartbeat; owner helper подтвердил UID/GID `10001`. Pre-canary status `33433536607`/`99624406068` повторно подтвердил healthy/isolation. После успешного STT и сбоя Google output confirmation worker был безопасно drained run `33433995948`/job `99625914547` at `2026-08-31T20:04:38Z`: exited/zero/gracefully-drained, same image/isolation. One approved reconciliation recovered the existing output, but recovery does not prove the original path regression absent. Local fix removes the invalid project-row lock under the restricted role; hosted PostgreSQL/CI/deploy/post-fix LIVE are pending. Worker remains stopped. Изменение `+66,7` п.п. — выполнение двух runtime AC, denominator не изменён.
...
Evidence: `SPEC ✅ | CODE ✅ | TEST ◐ | CI ◐ | DEPLOY ◐ | LIVE ◐`. Delivered account/accounting components passed exact CI and API deployment. One canary persisted `12.612426 s` and nominal `0.00077073 USD`; analytics explicitly separated this from invoice debit. Current account/product snapshot verifies subscription/period/remaining/reset/overage/invoice and product credits with explicit provenance, supporting `USAGEC-01/02/03/04/06`. Local branch adds the missing bounded per-job API/UI projection with explicit unavailable/partial/uncertain states and immutable tariff details; portable Python `1267 passed`, Studio `659 passed`, focused/lint/build checks pass. `USAGEC-05` remains uncompleted until current exact CI/deploy/LIVE. Initial Google confirmation failed, one approved reconciliation recovered the existing result, and the locally diagnosed lock fix remains undelivered. No second STT, provider retry or worker resume occurred.
