...
- Google Drive single file and Google Drive folder source modes.
...
- Existing Google Docs backfill uses temporary known defaults for historical transcript Docs: `Provider: ElevenLabs`, `Model: scribe_v2`, `Language: Русский`, and `Speakers: unknown`. Speakers are not inferred automatically.
