...
The current branch candidate Alembic head is `0020_partial_provider_checkpoints` under `apps/studio-api/alembic/versions/`, reached through direct additive predecessors `0018_job_part_progress` and `0019_job_media_clip`. The last exact production revision recorded in this document is `0017_google_maintenance_oauth`; later operator screenshots prove that progress and two-project split behavior reached a live runtime, but they do not independently establish an exact database revision or deployed image identity. Before any `0020` rollout, the protected lane must rediscover and verify the actual production revision, apply only its direct successor, and retain separate migration/API/worker evidence.
...
The bounded small-source Studio processing path is production-live with operator evidence: its original controlled canary ran against the `0015_user_source_retention` baseline with verified web/API identities, exactly one healthy worker from the commit-specific `900bf5b` image, and one operator-approved Drive source producing exactly one persisted `google_docs_transcript` and one non-empty native Google Doc without retry. Production PostgreSQL was subsequently protected by verified tagged backups and migrated through at least the separately recorded `0017_google_maintenance_oauth` gate; exact-main web/API and worker deployment/status evidence exists for `main@bd8d513`, and the operator completed later real batch and two-project split transcriptions. The live split run created both intended documents. A subsequent split workload then exposed the concrete partial-provider failure addressed by candidate `0020`: one internally prepared ElevenLabs part completed and a later part failed, while the browser retained only the aggregate `partial_provider_result` and offered no safe continuation. These facts prove only the observed gates; they do not prove broader workload stability, exactly-once behavior under arbitrary failures, the exact current production revision/image, the candidate `0020` resume path, or every maintenance outcome.
...
2. Production database migration head matches the intended repository head where required. The current branch head is the direct additive chain `0018_job_part_progress` then `0019_job_media_clip` then `0020_partial_provider_checkpoints`. The last exact revision retained in this document is `0017_google_maintenance_oauth`; live feature screenshots are not a substitute for rediscovering the exact database revision before a protected one-successor release.
...
- `PWA-TWO-PROJECT-SPLIT-01` — active branch work adds one optional pre-launch split of one source into two immutable complementary clip jobs, two separately verified Google output folders, server-side duration validation/clipping, clip-aware duplicate authority, and browser-safe history labels through `0019_job_media_clip`. Arbitrary cutting, more than two parts, editing after creation, and concatenation remain outside this item.
- `PWA-PARTIAL-PROVIDER-RESUME-01` — active branch work preserves the safe underlying ElevenLabs failure category plus encrypted, owner/job/source/part-scoped checkpoints for completed internal provider parts through `0020_partial_provider_checkpoints`. Only an explicit owner action may continue missing parts or, after unavailable/expired checkpoints and a safely classified failure, restart the full file; both actions require an explicit provider-cost confirmation. Automatic recovery, blind provider retry, browser transcript payloads, and reuse after output/cancellation/expiry remain prohibited.
