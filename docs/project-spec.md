...
The current branch candidate Alembic head is `0019_job_media_clip` under `apps/studio-api/alembic/versions/`, reached through direct additive predecessor `0018_job_part_progress`; operator-verified production PostgreSQL and merged `main` remain at `0017_google_maintenance_oauth`. The production `0017` rollout used a verified tagged backup and separately verified migration/API health. Candidates `0018`/`0019`, their API/worker consumers, the new progress UI, and the optional two-project split require their own protected rollout evidence.
...
2. Production database migration head matches the intended repository head where required. Production currently reports `0017_google_maintenance_oauth`; the current branch requires the direct additive chain `0018_job_part_progress` then `0019_job_media_clip`, while the bounded original processing canary ran against the older `0015_user_source_retention` baseline.
