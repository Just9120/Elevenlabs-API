...
Verified repository baseline: `main@7629fde8926d6af3758e7c1b8183188fa3c5244e` (recovery PR `#267`) имеет exact-main CI для worker isolation, usage/account implementation и diagnosed deployment recovery. API deployment verified at `bd51e756`; worker deployed/healthy on `8957e974`; web recovered on exact `7629fde8` by run `33408823378`. Public build identity, web health and API readiness confirmed at `2026-08-31T15:32:58Z`; earlier web `502` is resolved. Read-only status `33409097724` confirms unchanged healthy worker and `isolation_match=yes`. These deployment checks do not prove provider/account LIVE. Exact identifiers находятся в `docs/delivery-plan.md`.

Current operational Goal: `PWA-WORKER-USAGE-ACCOUNTING-01`; protected run `33360460155` migrated production to `0031_provider_account_snapshots` and deployed API at `bd51e7569370ec059b5926cee974c63a2d5c168f`. After operator SELECT grant and single-file master-key owner correction, worker run `33388441193` and separate status `33388535135` confirm deployment/health/resource and privilege isolation on exact `8957e974abe8c03224a572389c41aac84c99b609`. Owner diagnostics proved the internal-only web container had no actual host-port binding despite healthy nginx. PR `#267`, exact-main CI, real Docker host-publication regression test and web-only deployment `33408823378` resolved that failure while retaining loopback-only ingress and worker isolation. Read-only status `33409097724` confirms `isolation_match=yes` after exact CAP_ alias normalization; worker image is unchanged. Current Goal is `IN_PROGRESS` after explicit owner approval of the invoice compatibility fix: owner approved account refresh and one <=30-second transcription on 2026-08-31. After owner-reported User/Workspace Analytics permission update, the authenticated account panel shows unavailable data / `malformed_provider_response` instead of the earlier authentication rejection. Exact rejected field/response shape remains unverified. Polling was stopped; no transcription or source upload was submitted. Official InvoiceResponse compatibility defects were reproduced locally: missing/null invoice breakdown and the no-payment integer sentinel. The bounded API/web fix preserves nulls without fabricated zero values; targeted tests, full portable tests and frontend validation pass. Exact-head CI/deployment and account revalidation are still pending; no new AC is counted. New post-deploy evidence stays local/GitHub because the approved metadata writer is absent; no metadata-only follow-up PR, direct push, automatic rollback or secret rotation was performed. Goal авторизует `PWAWOR-02..03` и `USAGEC-01..06`: worker resource/privilege/network/DB-role isolation, exact provider-call accounting, immutable nominal tariff snapshot и official ElevenLabs subscription/workspace-usage reconciliation с explicit freshness. Commercial contour, пользовательский billing, realtime/storage costs, pricing-page scraping и unsupported credits-to-minutes/invoice allocation вне scope.
...
Evidence: `SPEC ✅ | CODE ✅ | TEST ✅ | CI ✅ | DEPLOY ✅ | LIVE ◐`. После operator master-key ownership correction worker deployment `33388441193`/job `99476297938` прошёл на exact `8957e974`; image `sha256:19d8c05f9de9777d87e5694673ed4a931591c5bbef7d577c1ee52fe10fcfa972`. Status `33388535135`/job `99476560035` подтвердил running/healthy, image/commit identity, 2 CPU, 4 GiB memory без дополнительного swap, 256 PID, read-only rootfs, no-new-privileges, две разрешённые сети и только CHOWN/SETGID/SETUID bootstrap capabilities. Required worker health проверяет restricted DB role/grants и current runtime heartbeat; owner helper подтвердил UID/GID `10001`. PR `#267` исправил false negative для эквивалентных `CAP_` aliases, сохранив strict allowlist. Post-web-recovery status `33409097724`/job `99543911523` at `2026-08-31T15:33:30Z` подтвердил unchanged healthy image, все прежние bounds и `isolation_match=yes`. Его `identity_match=unknown` относится только к отсутствующему worker tag нового web-only checkout `7629fde8`; actual image совпадает с previously verified worker `8957e974`, новый worker rollout не заявляется. Bounded end-to-end LIVE текущей Goal ещё не выполнен. Изменение `+66,7` п.п. — выполнение двух runtime AC, denominator не изменён.
...
Evidence: `SPEC ✅ | CODE ✅ | TEST ✅ | CI ✅ | DEPLOY ✅ | LIVE ❌`. API `bd51e756`/schema `0031`, worker `8957e974` и web `7629fde8` доставлены и прошли component health. After owner reported enabling User/Workspace Analytics permissions on 2026-08-31, the reloaded account panel shows `malformed_provider_response`, unavailable data and no fetched timestamp instead of the prior authentication rejection. Code confirms the error mapping, not the exact rejected field or HTTP status. Official schema/local normalizer comparison is insufficient to prove the runtime cause; bounded secret-free response-shape evidence is needed. Panel polling stopped, no manual retry or transcription created. Billed-duration canary remains not-run; AC не засчитаны по одним deployment checks.
...
- Verified repository main/VPS checkout: `7629fde8926d6af3758e7c1b8183188fa3c5244e`. API deployed at `bd51e7569370ec059b5926cee974c63a2d5c168f` and remains ready/schema `0031`. Worker deployed at `8957e974`, unchanged running/healthy image and verified isolation in status `33409097724`; rollback candidate preserved. Web-only recovery `33408823378` passed built/running image equality and localhost health; public build metadata confirms exact `7629fde8`, public web health and API readiness pass. Earlier `502` is resolved; no automatic rollback, migration or worker/API restart occurred during recovery.
...
2. Operator SELECT grant, master-key owner correction, restricted bootstrap/schema probes и worker rollout/inspection завершены. CAP_ classifier исправлен PR `#267`, runtime status подтверждает `isolation_match=yes`.
3. Web recovery после failed rollout `33388699857` завершён: owner diagnosis, reviewed fix, exact-main CI/real Docker regression и successful web-only CD `33408823378`; public build/health verified. API migration завершена и повторять её не нужно.
4. Owner approval для account refresh и одной <=30-second transcription получен 2026-08-31. После owner-reported permission update текущий account blocker — `malformed_provider_response`, точная причина ещё не подтверждена. Получить bounded secret-free response-shape diagnostic и исправить только подтверждённое расхождение; после успешной account revalidation выполнить ранее согласованный canary без второй задачи/automatic retry. Deployment health не подтверждает provider accounting.
