...
Current Studio non-goals/deferred boundaries remain: no worker, no queue consumer, no public processing pipeline, no output persistence, no manifest mutation, no production processing pipeline, no automatic migration rollout, and no production-live claim without runtime/operator evidence. Studio source now includes an internal single-transcript Google Docs creation boundary only; it is not a public or production processing pipeline. Studio job records are record-only until separately scoped worker/provider/output work changes that boundary.
