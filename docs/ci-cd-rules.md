...
- **Постоянный remote metadata mechanism: UNSET для будущих Goals.** В проверенных repository workflows такой automation нет. Для текущей Goal действует отдельное одноразовое решение ниже. Механизм будущей Goal, его target, permissions и safety guarantees проверяются при её подготовке; разрешение текущей Goal на них не переносится. Новая automation и обход protections не разрешены.
- **Одноразовый механизм AUDIO-NAME-ERROR-UX-01 разрешён владельцем 2026-09-06.** Источник — ответ «ну делай» в текущем чате на предложение одного metadata commit/push напрямую в main и объяснение оставшегося шага; opaque message ID не предоставлен. Один ordinary fast-forward metadata commit/push в Just9120/Elevenlabs-API:main после verified product delivery 84ae25f. Allowlist: docs/delivery-plan.md, docs/delivery/studio.md и только Project profile данного файла. Preconditions: fresh main SHA, protections/rulesets и full allowlist/diff check; при concurrent update перечитать base, при protection gate остановиться. Force/protection bypass, изменение общих safety rules, credentials/settings/workflows и новая automation запрещены. После записи: remote file/commit readback, local main synchronization и применимый обычный CI. Разрешение ограничено текущей Goal; это не постоянная policy для будущих Goals.
- Новые общие правила явно утверждены владельцем 2026-09-06 в ответе «Да, применить обе новые версии». Закрытая поставка #301 описана по действовавшему тогда DoD. Исходный AUDIT завершился локальными документами; последующая AUDIO-NAME-ERROR-UX-01 отдельно разрешена ответом «Запускай» и поставлена через #302. Фактические records и финальный metadata checkpoint — в delivery-plan.
