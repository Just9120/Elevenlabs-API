...
- 👉 `PWA-API-LEAST-PRIVILEGE-01` — Next local item. Run the API/worker image as an explicit unprivileged account while retaining read-only secret access, writable temporary storage, health checks, and the worker command contract.
