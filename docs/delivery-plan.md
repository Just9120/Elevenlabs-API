...
- 👉 **PWA-JOBS-01B — Studio transcription job frontend UI** — active frontend/docs item: add platform-mode per-project job create/list/detail/cancel UI for queued/cancelled job records from existing uploaded project sources only. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no backend schema/API changes unless a frontend-blocking contract bug is found, no production deployment, and no automatic migration rollout.
...
The active Studio frontend/docs item is PWA-JOBS-01B: add the smallest platform-mode per-project UI for transcription job records. It uses existing sources and existing job APIs to list jobs, create queued jobs from selected uploaded sources, inspect ordered safe source detail, and cancel queued jobs. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no backend schema/API changes unless a frontend-blocking contract bug is found, no production deployment, and no automatic migration rollout.
