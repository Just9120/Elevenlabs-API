...
- 👉 **PWA-JOBS-03A — Internal Studio job claim/lease persistence and fencing foundation** — active/in PR: source-only PostgreSQL lease metadata plus transactional acquire/renew/release primitives with generation fencing. Jobs remain record-only and claimed jobs stay `queued`; this slice adds no worker, queue consumer, provider execution, source access, Google Docs output, output persistence, manifest mutation, production rollout, VPS access, CI/CD change, Docker/runtime/deploy change, or secrets change. Production migration application remains manual/operator-scoped.
...
The active recommended item is **PWA-JOBS-03A — Internal Studio job claim/lease persistence and fencing foundation**. It is a source-only backend foundation that adds internal lease metadata and transactional acquire/renew/release helpers for one eligible queued Studio job. Lease ownership is exclusive intent for future work only: jobs remain record-only, claimed jobs remain `queued`, and `started_at` stays reserved for a later real processing transition.
