...
- 👉 `PWA-PROCESSING-ROLLOUT-01A / Gate 6 authorization` — Gate 5 deploy success is not a production-live claim. Run exactly one bounded canary only after separate explicit operator authorization; do not manually retry or submit a second job when provider/Google side-effect state is uncertain.
...
- Studio has broad source-level implementation at merged and deployed `main` revision `900bf5b`. Gates 4–5 are complete; the dominant remaining production proof is one controlled canary, not missing core ElevenLabs processing code.
...
- Exactly one worker is now running healthy from the commit-specific `900bf5b` image. Deploy run `30107907971` and post-status run `30107810563` prove database compatibility, Docker health, and `identity_match=yes`; the PWA showed no current jobs after rollout. No canary has run, so do not describe processing as production-live.
...
| Studio UX readiness | **about 89%** | Both Picker roles, single-attempt local upload/completion, prerequisites, asynchronous source-removal feedback, and the PWA under the production CSP now have clean authenticated public evidence. Processing-result UX still awaits the controlled canary. |
| Studio production evidence | **about 91%** | Backup, migration, current API/web/worker identities, database compatibility, services, public health, the clean Gate 4 smoke, TLS, live security headers, and one healthy idle worker are evidenced. The one-output canary and stabilization checkpoint remain. |
| Studio combined v1 readiness | **about 87% deployed** | The public browser/API boundary and exactly one intended worker are ready. One bounded canary and post-canary stabilization remain before processing can be described as production-live. |
...
- Gates 4–5 are complete. The one-output canary remains unproven and unauthorized pending separate Gate 6 approval.
