...
- 📋 `DOCS-AUTHORITY-SYNC-02` — Reconcile stale source-complete versus production-rollout claims and consolidate pointer-only documents — Status claims synchronized in the local batch; pointer consolidation remains pending.
...
- `DOCS-AUTHORITY-SYNC-02` — consolidate `docs/provider-transcription-contract.md` and `VALIDATION_MATRIX.md` into their current authorities after the local status-claim reconciliation.
