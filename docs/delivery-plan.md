...
- 👉 **PWA-PROCESSING-CONTRACT-01 — Studio output/source/manifest parity contracts before worker implementation** — active/in PR: documentation-only product/runtime contract alignment for output transcripts, processing-time source access, and Studio/Colab manifest authority. Explicit non-goals: no code changes, no new docs files, no worker/provider execution, no queue consumer, no Drive download/export implementation, no Google Docs API calls, no output persistence, no manifest mutation implementation, no migrations, no CI/CD workflow changes, no Docker/runtime/deploy changes, no production rollout, and no secrets.
...
Studio PWA is the current development contour intended to duplicate Google Colab product scope with PWA/platform adaptations. Source on main now includes account/session/BYOK, projects, sources, Google Drive OAuth/metadata/folder-child selection, local temporary upload intake, persisted job records, job UI, and preflight/claim-readiness guardrails. Current Studio jobs remain record-only: no worker/provider execution, queue consumer, Drive download/export processing, Google Docs output, output persistence, production processing pipeline, or manifest mutation is claimed. Studio job persistence migration rollout remains manual/operator-scoped unless operator evidence exists.
...
The active recommended item is **PWA-PROCESSING-CONTRACT-01 — Studio output/source/manifest parity contracts before worker implementation**. It is a focused documentation-only PR that defines Studio PWA parity contracts for Google Docs transcript output, processing-time source access, and Colab/Studio manifest authority before any worker/provider implementation. It updates existing source-of-truth documents only and does not create new docs files.

Explicit non-goals: no application code changes; no new documentation files; no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Drive download/export implementation; no source byte access; no Google Docs API calls or output creation; no output persistence; no manifest mutation implementation; no job claiming/leasing implementation; no retry runner/background tasks; no public API endpoint; no backend schema changes or Alembic migrations; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, or presigned URLs.
