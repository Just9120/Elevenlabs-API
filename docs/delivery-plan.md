...
- 👉 `PWA-TRANSCRIPT-MAINTENANCE-SPLIT-01 / product contract` — The user explicitly split the old combined migration into two operations: existing-document standardization and Studio catalog import. Each must have its own explicit document selection, dry-run, and apply confirmation. Standardization may mutate only selected Google Docs; catalog import may mutate only PostgreSQL catalog metadata. Both preserve `drive.file`; no apply ran.
...
1. Implement the decided split: preserve `drive.file`, keep one selected-folder boundary, and require bounded explicit Picker selection of the Google Docs for each independent operation.
2. Separate API dry-run/apply behavior and tests so standardization cannot persist catalog metadata and catalog import cannot issue Google writes; reject unselected, duplicate, cross-folder, changed, or over-limit document IDs.
3. Redeploy the applicable components, repeat one approved-document dry-run for each operation, review only safe aggregate results, and require separate authorization for each bounded apply.
