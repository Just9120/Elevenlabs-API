...
- Studio PWA production processing is not claimed: current jobs remain record-only, with no worker, queue consumer, provider execution, Drive download/export processing, Google Docs output, output persistence, production processing pipeline, or manifest mutation.
...
- 👉 **STUDIO-CD-IMAGE-01 — Enforce component image replacement and identity verification** — active/in PR operational fix: make Studio platform component CD force-recreate only the selected stateless component and verify the running container image ID matches the newly built mutable local image tag before localhost health can produce a success marker. Observed production evidence showed a mutable local image tag was rebuilt while the previous healthy container remained running, so HTTP health alone was insufficient proof of deployment. Automatic CD validation is not complete until post-merge operator evidence exists.
- 📋 **PWA-OUTPUT-01A — Internal Google Docs single-transcript write boundary** — next product item after the CD image-identity fix.
...
Studio PWA is the current development contour intended to duplicate Google Colab product scope with PWA/platform adaptations. Source on main now includes account/session/BYOK, projects, sources, Google Drive OAuth/metadata/folder-child selection, local temporary upload intake, persisted job records, job UI, preflight/claim-readiness guardrails, internal processing-time source materialization, and processing prerequisites. PWA-JOBS-04B has merged its internal ElevenLabs single-source execution boundary; current Studio jobs still have no worker, public processing pipeline, Google Docs output, transcript/output persistence, completion transition, production processing pipeline, or manifest mutation claimed. The active PR is STUDIO-CD-IMAGE-01, an operational CD correctness fix for Studio platform component image replacement and identity verification. Studio job persistence migration rollout remains manual/operator-scoped unless operator evidence exists.
...
The active recommended item is **STUDIO-CD-IMAGE-01 — Enforce component image replacement and identity verification**. It is an operational CD correctness fix for `scripts/deploy_studio_platform_component.sh`: after building the selected component image, capture the built mutable local image ID, force-recreate only the selected stateless component with `--no-deps`, verify the running container image ID exactly matches the captured built image ID, then run localhost health and print the success marker only after both checks pass.

After STUDIO-CD-IMAGE-01, the next product item remains **PWA-OUTPUT-01A — Internal Google Docs single-transcript write boundary**.

Explicit non-goals for STUDIO-CD-IMAGE-01: no product behavior change; no provider execution; no Google Docs output; no worker or queue; no public API or frontend change; no models or migrations; no PostgreSQL, Redis, volume, backup, restore, nginx, runtime `.env`, Repository Secret/Variable, SSH access, Docker base image, dependency, retry, rollback, cleanup, hardening, or VPS mutation scope.
