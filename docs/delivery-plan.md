...
Continue local PWA stability hardening by converging the duplicate Google connection consumers outside Settings. Audit Overview and Projects connection reads for runtime validation, request deadlines, stale-response ordering, teardown cancellation, and contradictory UI state; prefer one narrow shared read contract only where it reduces proven duplication without broad component churn. Keep OAuth mutations, maintenance OAuth, Picker interaction, API/schema, deployment, and production state outside this item.
