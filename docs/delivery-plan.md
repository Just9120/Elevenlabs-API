...
- 👉 **PWA-SOURCE-SELECTION-UX-01 — Clarify source removal and preserve Unicode filenames** — current focused frontend/backend/docs item: source removal wording currently implies possible external-file deletion, soft-deleted project sources remain visible in the active list, and Unicode filenames are mangled by an ASCII-oriented persistence helper. Google Drive files must never be deleted by source removal; removed sources disappear only from active project selection; historical job references remain; Unicode display names are stored separately from technical transport-safe names. Worker startup, production processing smoke, provider requests, Google Docs creation, exactly-once output validation, cleanup, rollback, and manifest parity remain separate.
...
## Last completed item: PWA-DRIVE-LINKS-UX-01 — Approved Google Drive resource links

Status: source-done/merged into main via PR #143 (head `2421f6e3d306bb770bd6d0a81e229d23fa2c2df8`, merge `62137df92a626048970d3c0f29ffeff69c6fb8a4`). CI #339 passed and Studio PWA CI #164 passed.

Latest supported production evidence:

- Confirmed: the compact Drive-link UI is deployed and visible.
- Confirmed from the previous Picker item: destination folder selection works.
- Confirmed from the previous Picker item: selected destination folder persistence works.
- Confirmed from the previous Picker item: source Picker displays media files.
- Confirmed from the previous Picker item: a `.flac` source can be persisted.
- Confirmed from the previous Picker item: previous source/folder Picker blockers are cleared.
...
**PWA-SOURCE-SELECTION-UX-01 — Clarify source removal and preserve Unicode filenames** is the active focused frontend/backend/docs item. It clarifies that Google Drive source removal only removes the source from the Studio project and never deletes, trashes, renames, or otherwise mutates the external Drive file; hides soft-deleted sources from the active project source list and new-job selection; preserves historical job source references; and separates Unicode-preserving display filenames from technical transport-safe filenames used only at materialization/provider boundaries. Observed defects: source removal wording implies possible external-file deletion, soft-deleted project sources remain visible in the active list, and Unicode filenames are mangled by an ASCII-oriented persistence helper. This item does not start workers, run production processing smoke, call providers, create Google Docs, validate exactly-once output, perform cleanup, perform rollback, or claim manifest parity.
