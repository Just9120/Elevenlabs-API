...
- 🔄 `DEPENDENCY-AUDIT-RUN-IDENTITY-01J` — Scheduled/manual audit runs now expose their ref and full SHA in the run name, while the runbook retains `headSha` as the authoritative exact-revision check.
- ✅ `BATCH-PRE-PR-GATE-01` — The complete fifteen-commit `main...HEAD` diff and commit series were reviewed without finding scope drift, hidden product behavior, secret material, or a local release blocker. The full applicable local gate passes; GitHub CI and the exact-revision remote dependency audit remain authoritative.
- 👉 `BATCH-PUBLISH-01` — Publish the exact fifteen-commit branch, open its PR, and wait for every required check. After publication, dispatch the unchanged dependency-audit workflow against the exact branch SHA; do not mark the dependency blocker cleared from local evidence alone.
...
- `main` and `origin/main` are `7362a0c` (PR #187). The active `codex/ci-actions-runtime-maintenance` branch starts from that exact revision; its publication-ready batch consists of fourteen focused implementation/evidence commits plus this pre-PR checkpoint.
...
- PR #187 local validation included full ESLint, TypeScript, production Vite/PWA build, Playwright discovery, focused contracts, and portable Python. The current CI-maintenance batch additionally passes YAML parsing for all seven workflows, the focused action-runtime contracts (`14 passed, 2 skipped`), the focused CD-observability contracts (`7 passed`), the dependency-audit contracts (`9 passed`), `723 passed, 6 skipped` portable Python, lightweight repository checks, and `git diff --check`. Its final clean gate repeated exact lock installation, a zero-vulnerability audit, ESLint 10, all 276 Vitest tests, TypeScript, the production Vite/PWA build, Playwright discovery (`8 tests`), portable Python, and lightweight checks. A complete review of all 17 changed files found no scope drift, hidden product behavior, secret material, or local release blocker. The Windows sandbox requires Vite's supported module-runner config loader and an isolated pytest temp directory; GitHub CI remains authoritative for default Linux execution, shell/deploy integration, and the upgraded actions. The first ordinary qualifying post-merge CD run remains the runtime-summary evidence boundary.
