...
- 👉 **PWA-JOBS-04B — Internal ElevenLabs single-source transcription boundary** — active/in PR: composes existing processing prerequisites, one materialized source, one synchronous ElevenLabs `scribe_v2` request, immediate pre-provider and post-provider DB-only revalidation, and an ephemeral redacted transcript result. It still adds no worker, public processing pipeline, public endpoint, OpenAI support, Google Docs output, output persistence, job completion, manifest mutation, migration, CI/CD/runtime/deploy/production rollout, VPS access, or secrets change.
...
The active recommended item is **PWA-JOBS-04B — Internal ElevenLabs single-source transcription boundary**. It is an internal-only boundary for exactly one already-materialized source of an already-leased `processing` job. It requires the existing prerequisites handle to be ElevenLabs, performs fresh DB-only identity/lifecycle revalidation immediately before the single synchronous `scribe_v2` provider request, performs a post-provider lifecycle revalidation before transcript access, and yields only an ephemeral redacted transcript result.

Explicit non-goals: no new documentation files; no worker implementation; no queue consumer; no public processing pipeline; no public API endpoint; no OpenAI support; no media conversion/chunking/diarization additions; no Google Docs API calls or output creation; no transcript/output persistence; no job completion transition; no job-source status persistence; no manifest mutation implementation; no retry runner/background tasks; no new job status values beyond existing `queued`/`processing`/terminal states; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no VPS access; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, Drive IDs, or presigned URLs.
