...
- `main` and `origin/main` were synchronized at `625cd33` after PR #190. The current documentation-reconciliation branch starts from that exact revision and changes no product/runtime surface.
- The merged catalog source passed its final local gate with `788` portable Python tests and `6` skipped, all `285` frontend unit/component tests, ESLint, TypeScript, production build, lightweight repository checks, and `git diff --check`; Playwright discovery lists `9` authenticated scenarios. The documentation audit reran lightweight checks and the same portable Python suite successfully. PostgreSQL/Redis-backed API and authenticated Chromium behavior remain GitHub CI authority and are not replaced by local SQLite evidence.
