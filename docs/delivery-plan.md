...
- 👉 `PWA-TRANSCRIPT-MAINTENANCE-SPLIT-01 / PR validation` — The local candidate now presents existing-document standardization and the user-facing `Манифест Studio` operation as two independent operations. The manifest label maps to PostgreSQL catalog metadata and does not imply a separate file. Each operation has its own folder and explicit-document Picker selection, dry-run state, confirmation, apply route, rate-limit/audit boundary, and result. Standardization has no catalog persistence capability; catalog import has no Google-write capability. The old combined routes return a safe `410 transcript_maintenance_split_required`. Local targeted/backend and full frontend gates pass; exact-main CI, deployment, and real authenticated Google evidence remain. No apply ran.
...
- Studio has broad source-level implementation at merged `main` revision `202deed`. Exact-main repository run `30299809500` and Studio/browser run `30299809479` are green for that revision.
...
- Repository head, production PostgreSQL, and the deployed API agree on `0016_transcript_catalog_entries`. The merged public edge repair is also live: local and public TLS expose only the production origin, without path/query data, public web/API health remains green, and authenticated Chrome opens the Picker list without the prior invalid-key failure. Production then exposed a catalog authorization-design blocker: folder-only Picker selection under `drive.file` did not make the existing child documents discoverable to the server.
- The current local branch addresses that blocker in source by requiring the user to select the direct-child Google Docs explicitly for each of two independent operations. This candidate is neither merged nor deployed, and no real-document dry-run or apply has been attributed to it.
...
| Selected Studio v1 source/CI | **96% authoritative main / 98% local candidate** | Main remains `38.5/40 = 96.25%`, rounded to 96%. On the same ten-epic denominator, the local split candidate now passes contract/design, source, and targeted-test gates for the affected epic, while exact-main CI is still absent: `39/40 = 97.5%`, rounded to 98%. |
| Selected Studio v1 production evidence | **61%** | Completed backup/schema and intended API identity/health evidence remains valid. The two zero-item dry-runs add no catalog gate, while the blocker does not erase the completed infrastructure evidence. |
...
| Catalog migration production rollout | **60% (`3/5`), blocked** | The existing source/UI prerequisite, verified backup plus `0016`, and intended API deployment/identity/health remain the three completed evidence gates. The separated candidate is not deployed; meaningful selected-document dry-runs and separately authorized applies remain unproven. |

The scoring method and partial-gate rule are in `docs/audits/repository-audit-2026-07-26.md`. Exact-main `202deed` checks remain the authoritative merged evidence. The local candidate recovers only the catalog contract/source/targeted-test gates; it cannot receive the exact-main CI gate before PR merge and exact-main validation. Catalog rollout remains `3/5` and Studio production evidence remains 61% because local code and successful zero-item requests are not deployed approved-document evidence.

## Product roadmap after production proof

Order work so each capability inherits a known source and production baseline:

1. Publish the focused split candidate, pass all PR checks, review the complete `main...HEAD` diff, and merge only after the source/CI gate is green.
2. Deploy only the affected web/API components under the CI/CD safety contract and verify exact revision, image identity, database revision, and public health.
3. In the user's authenticated Chrome, run one non-mutating approved-document dry-run for standardization and one separate dry-run for catalog import; review only safe aggregate results.
4. Require a separate explicit authorization for each bounded apply. Never use one operation's preview or confirmation for the other.
...
- Local `main`, `origin/main`, and the clean VPS checkout are synchronized at merge revision `202deed`.
- Exact-main repository CI run `30299809500` and Studio run `30299809479` passed at `202deed`, including full PostgreSQL/Redis pytest, Alembic, frontend checks/build, authenticated browser E2E, both image builds, protected-secret bootstrap smoke, and Compose validation. Standard component CD did not run because the merged application-path filters exclude the explicit operator-owned host-nginx surface.
...
- Authenticated Chrome opened the production Google Picker list without the prior invalid-key failure under the live `origin` policy. Two explicitly selected folders then produced successful, non-mutating dry-run responses with all scan counters at zero; the user confirmed that the second folder contains files directly at its root. Repository tracing found the server querying folder children while the durable OAuth contract remains `drive.file` and Picker grants authority only to the folder. Google's [`drive.file` scope guide](https://developers.google.com/workspace/drive/api/guides/api-specific-auth) describes per-file access for files opened or shared through Picker, so the observed result is consistent with missing child-document authority rather than an empty folder. The user has now resolved the product decision: standardization and catalog import are separate explicit-document operations. Apply was not performed and remains prohibited until the split is implemented, deployed, and the corresponding meaningful dry-run is reviewed.
- The current local candidate implements that decision end to end: bounded explicit document selection, independent plan/dry-run/apply services and routes, separate PWA cards/buttons/state, and fail-closed legacy combined routes. The focused backend suite passed `92` tests; the full Studio frontend suite passed `297` tests across `28` files; ESLint, TypeScript, production build, Python compilation, and lightweight checks passed. The authenticated Playwright suite still discovers `9` scenarios from the prior branch gate. Service-backed PostgreSQL/Redis tests, PR/exact-main CI, deployment identity, and the user's authenticated Chrome/Google dry-runs remain pending; no production apply or other Google mutation ran.
- PR #194 automated review identified that `docs/project-spec.md` still described production `0016`, intended API deployment, and browser CI as unproven. The current branch reconciles those factual status annotations with verified evidence without changing scope, business rules, or acceptance criteria. The old PR thread remains open because replying or resolving it requires a separate explicit GitHub write instruction.
