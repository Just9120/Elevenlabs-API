...
`apps/studio/` static mode remains demo-only and does not call `/api`. Platform mode now has Projects/source UI for the already-live projects and sources backends: project list/create/edit/archive, selected output Drive folder metadata, source listing/deletion, Google Drive source metadata creation from verified safe backend metadata, temporary local-upload intake, and Google Drive connection status/start/disconnect UI on account settings. PWA-GOOGLE-04A is source-done/merged on main. The current frontend/docs follow-up is PWA-GOOGLE-04B: list safe normalized metadata for direct children of one explicitly supplied Drive folder ID in the Sources UI, select file-like children, and create source records from selected safe metadata. Jobs, providers, Drive picker, recursive browsing, Drive search, Google Docs output, processing, queues/workers, output persistence, sharing, production deployment, and manifest mutation remain deferred. PWA-GOOGLE-01B is source-done/merged for runtime config wiring, but production rollout/live enablement is not claimed without operator evidence.
...
The active Studio frontend/docs item is PWA-GOOGLE-04B: platform-mode Studio should let users list safe normalized direct children for one explicitly supplied Google Drive folder ID, select file-like children, and create Google Drive source records from selected safe metadata. Explicit non-goals: no Drive Picker, no recursive folder browsing, no Drive search, no Google Docs creation, no provider execution, no jobs/queues/workers, no migration, no production deployment, and no manifest mutation.
