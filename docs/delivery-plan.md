...
- 👉 `PWA-CD-OBSERVABILITY-01` — Make component selection, migration-blocked API skips, manual-only worker decisions, and final per-job results explicit in the Studio Platform CD job summary. Local implementation and contracts are in the current batch; PR CI is the source acceptance gate, while the first ordinary qualifying CD run is the runtime-summary acceptance gate.
