...
- 🧪 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01A/01B/01C` — The current branch now connects the duplicate-authority foundation to batch preflight/create for accepted Studio outputs. Exact, matching-settings legacy-standard, and indeterminate evidence block a row until the user explicitly chooses paid reprocessing; no automatic skip, reuse, overwrite, or provider call occurs. Source or effective-settings edits clear the decision, the flag participates in idempotency, and create repeats the query after deterministic owner-scoped catalog-identity locks, including every Studio row for a reselected Google Drive file. The affirmative choice is now durable per job as a reserved server-owned option that is not returned to the browser and cannot be injected through the deprecated arbitrary-options route; that route now blocks accepted-evidence conflicts and sends callers to canonical batch preflight/create. The browser receives only safe categories/counts and an honest `studio_outputs_only` partial-authority marker. No migration, Google/provider call, document read, schema change, or continuous synchronization is introduced. Historical Drive discovery/import, in-place standardization, local-copy fingerprinting, and accepted-output reuse/linkage remain follow-up scope. Full PWA tests (`257 passed`), portable Python (`697 passed, 6 skipped`), focused pure catalog/options/preflight tests (`10 passed`), TypeScript, lint, production build, compile, lightweight, and diff checks pass locally. The PostgreSQL conflict/lock and legacy-route endpoint tests are authored but cannot execute without the unavailable local PostgreSQL service; service-backed CI remains authoritative.
...
| Studio source breadth | **about 97%** | The current branch adds typed transcription options, validated multi-source intake, video/long-media preparation, safe batch preflight, checkpoint-based staged progress, aggregate analytics, and explicit duplicate decisions for accepted Studio outputs. Historical Drive catalog import/standardization, accepted-output reuse, finer optional preparation telemetry, and rollout evidence remain. |
...
| Studio combined v1 readiness | **about 82% ±5** | Weighted planning estimate across product, parity, quality, operations, and production evidence. |
