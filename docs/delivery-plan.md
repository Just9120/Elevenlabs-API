...
- ⛔ `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / authorization design` — Two authenticated production dry-runs completed without mutation and returned safe zero-item previews. The user confirmed that the second explicitly selected folder contains files directly at its root. The deployed flow limits OAuth to `drive.file`, grants Picker authority only to the selected folder, and then asks the server to list pre-existing children for which it has no per-file authority. No apply ran. The recommended next design preserves `drive.file` and obtains explicit bounded document selections through Picker; changing this product flow or broadening OAuth requires an explicit decision before `docs/project-spec.md` or source changes.
...
| Selected Studio v1 source/CI | **96% authoritative** | Exact-main CI remains green, but the catalog contract/design gate is no longer met after real Google evidence disproved the folder-only authorization composition. On the same ten-epic denominator, `38.5/40 = 96.25%`, rounded to 96%. |
...
| Catalog migration production rollout | **60% (`3/5`), blocked** | Source/UI presence, the verified backup plus `0016`, and intended API deployment/identity/health remain the three completed evidence gates. Existing-document classification and separately authorized apply remain unproven because the current Picker/scope composition cannot discover the intended files. |

The scoring method and partial-gate rule are in `docs/audits/repository-audit-2026-07-26.md`. Exact-main `202deed` checks still back the implementation/test/CI gates, but the newly disproved catalog authorization design lowers the authoritative source/CI estimate from 99% to 96%. Catalog rollout remains `3/5` and Studio production evidence remains 61% because a successful zero-item request is not approved-document migration evidence.

## Product roadmap after production proof

Order work so each capability inherits a known source and production baseline:

1. Obtain an explicit product decision for catalog authorization. The recommended design preserves `drive.file`, keeps one selected-folder boundary, and requires bounded explicit Picker selection of the Google Docs to be inspected; broad `drive.readonly`/`drive` scopes are not the default.
2. After that decision, update `docs/project-spec.md`, implement the selected-document authority contract across Picker/API/apply, and add tests that reject unselected, duplicate, cross-folder, changed, or over-limit document IDs.
3. Redeploy the applicable components, repeat one approved-document dry-run, review only safe aggregate results, and require separate authorization for one bounded apply.
