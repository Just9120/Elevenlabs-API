...
- ✅ **PWA-BATCH-COMPOSER-ROW-UX-01 — Inline source and destination selection inside repeatable composer rows** — source-done in this branch; changes `Подготовка` to row-first source→folder selection, Google Drive/local multi-select row expansion, and secondary project-file management while preserving explicit atomic `/jobs/batch` submission.
- 👉 **PWA-PROCESSING-ROLLOUT-01A — Manual Studio processing rollout and controlled smoke validation** — manual-ops-required: previous operator rollout validation recorded migration-applied/API-health/worker-idle evidence, but composer web deployment, browser acceptance, and controlled production processing smoke remain separate and unverified; no production-live processing claim is made.
...
Status: source-done in this branch. The normal project workspace still exposes only `Обзор` and `Подготовка`; `Подготовка` now starts from repeatable rows that combine source selection and destination-folder selection. Google Drive and local multi-selection expand into ordered rows automatically, existing usable project sources remain row-selectable, source management is secondary, and explicit `/jobs/batch` submission remains required.
...
- Not yet confirmed after PR #146 merge commit `16ecbf2a6f23180bacdf412fa94901069806aea0`: the composer frontend has not been factually confirmed as deployed or visible in production.
