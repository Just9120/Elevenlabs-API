...
- 👉 **PWA-WORKER-01A — Internal single-iteration claim-next-and-process boundary** — active implementation item only; add one internal synchronous invocation that atomically selects the oldest unlocked ready queued job in PostgreSQL, acquires and commits its lease, and invokes the existing synchronous orchestrator once, returning `None` when idle.
...
The active recommended item is **PWA-WORKER-01A — Internal single-iteration claim-next-and-process boundary**. It implements only one internal synchronous invocation that uses PostgreSQL row locking to select the oldest currently unlocked ready queued job, acquire and commit its lease, and invoke the existing synchronous orchestrator once, returning `None` when idle. It must not add polling, sleeping/backoff, a worker, queue consumer, scheduler, public processing API, output browser API, manifest behavior, automatic retry/reconciliation, runtime/deploy changes, production migration execution, or production-live processing claim.
