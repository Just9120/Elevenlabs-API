...
- 🧪 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01A` — The current branch adds a server-only duplicate-authority foundation over existing accepted Studio outputs. It canonicalizes effective PWA settings and classifies owner-scoped evidence as exact, requiring in-place standardization, indeterminate, or different-settings/no-match. Google Drive identity is stable across reselected Studio source rows; local-upload identity remains the existing source row because no content fingerprint is stored. The foundation performs no migration, Google/provider call, document read, skip, reprocess, or browser change, and preflight must continue reporting catalog authority unavailable until the explicit conflict UX plus atomic create-time recheck exist. Focused catalog/provider/preflight/output tests (`116 passed`), portable Python (`696 passed, 6 skipped`), compile, and diff checks pass locally. An owner-isolation/current-output query test is authored; its PostgreSQL execution and later preflight/create integration remain service-backed CI/follow-up gates.
...
| Studio source breadth | **about 96%** | The current branch adds typed transcription options, validated multi-source intake, video/long-media preparation, safe batch preflight, checkpoint-based staged progress, and aggregate analytics to the existing ElevenLabs workflow and lifecycle foundation; catalog-backed duplicate decisions, finer optional preparation telemetry, and rollout evidence remain. |
...
| Studio combined v1 readiness | **about 81% ±5** | Weighted planning estimate across product, parity, quality, operations, and production evidence. |
