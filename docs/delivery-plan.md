...
- 👉 `PWA-TRANSCRIPT-MAINTENANCE-RECURSIVE-02 + TARGET-MODES-03 / local source candidate` — Standardization and `Манифест Studio` now own independent dropdowns and targets: one bounded recursive folder tree or exactly one Google Doc. Requests require the matching mode and exactly one ID; dry-run and apply freshly revalidate that target, skip target-state documents, isolate safe per-document failures, and use a separately consented/stored server-only maintenance OAuth grant. The maintenance subject must match the primary Picker account; its tokens never reach the browser or worker. Source tasks and the full applicable local gate pass; exact-head CI, migration `0017_google_maintenance_oauth`, runtime secret/config rollout, deployed identity, authenticated target-mode dry-runs, and separately approved applies remain.
- 📋 `PWA-TRANSCRIPT-MAINTENANCE-RECURSIVE-02 + TARGET-MODES-03 / rollout` — After merge: prepare a new tagged backup, apply and verify `0017`, configure the separate maintenance OAuth client/secret on the VPS, deploy exact web/API images, complete same-account consent, then run one safe dry-run per operation and target mode. Apply requires a later explicit decision for each bounded canary.
...
- Merged main, production PostgreSQL, and the deployed API agree on `0016_transcript_catalog_entries`. The merged public edge repair is live, and PR #195's explicit-document split is deployed. Neither fact proves the newly selected recursive traversal or the current strict target-mode contract.
- The current local branch combines bounded recursive traversal and exact single-document revalidation behind independent per-operation modes, plus a separate server-only maintenance grant and repository migration `0017_google_maintenance_oauth`. It is neither merged nor deployed; no dry-run or apply has been attributed to this contract.
...
The scoring method and partial-gate rule are in `docs/audits/repository-audit-2026-07-26.md`. Exact-main `5ab3b5f` checks remain authoritative for merged source, while current-branch targeted evidence covers only the maintenance candidate. Documentation, local code, and prior zero-item requests cannot increase production readiness.

## Product roadmap after production proof

Order work so each capability inherits a known source and production baseline:

1. Publish the completed local maintenance target-mode batch as one PR, inspect the exact-head checks, and merge only after CI is green.
...
4. In the user's authenticated Chrome, run a bounded non-mutating dry-run matrix for both operations: one recursive-folder target and one single-document target per panel; review only safe results.
5. Require a separate explicit authorization for each bounded apply. Never use one operation's mode, target, preview, confirmation, or result as authority for the other.
...
- The current local candidate combines separate encrypted maintenance consent, exact same-account/scope validation, server-only refresh, bounded recursive traversal, exact single-document revalidation, strict `selection_mode` plus one-ID request schemas, per-document isolation, and independent PWA dropdown/target state. The final local gate passed: lightweight checks and ESLint were clean, the portable Python suite reported `881 passed, 6 skipped`, all `313` Vitest tests across `28` files passed, the `127` affected panel/App tests passed, TypeScript and the production build passed, Python compilation and diff checks passed, and Playwright discovered the existing `9` E2E scenarios. The first portable run used sandbox-inaccessible `%TEMP%` and produced setup-only `PermissionError` results; the authoritative rerun used a workspace-local temporary base and passed completely. Exact-head Linux/Docker CI remains the source/CI gate not supplied by local evidence.
- Repository candidate head is `0017_google_maintenance_oauth`, while production remains operator-evidenced at `0016_transcript_catalog_entries`. The new runtime client ID/secret/scopes are not configured or deployed, no same-account maintenance consent exists in production, and no dry-run/apply under the current target-mode contract has run. Those are explicit rollout blockers, not source defects.
