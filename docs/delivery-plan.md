...
- ✅ `PWA-PREFLIGHT-UNCERTAINTY-01` — PR #189 merged at current `main` revision `95d3210`. PR and post-merge repository/Studio CI passed, including service-backed PostgreSQL and authenticated Chromium. Component CD deployed the changed web and API components, verified their running image identities and localhost health, skipped the manual-only worker, and public web/API health passed.
...
- 🚨 `PWA-PROJECT-CREATE-NAVIGATION-RACE-01` — Active item. Post-merge Studio PWA CI run `30202031078` failed its first authenticated Chromium scenario because the project form did not remain open after the explicit `Новый проект` click; eight later scenarios did not run. Fix the pending `browse` navigation intent versus explicit create-action race and restore a green exact-main browser run before treating the catalog UI baseline as accepted.
- 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Next item after browser CI is restored: tagged pre-migration backup, manual migration to `0016_transcript_catalog_entries`, intended API deployment/identity/health verification, authenticated approved-folder dry-run, and a separately authorized apply.
...
- Studio has broad source-level implementation at merged `main` revision `95d3210`. Gates 4–7 remain complete: the real one-output ElevenLabs-to-Google-Docs path is production-proven, the result-status defect is deployed and authenticated-live verified, PR #187 supplies repeatable secretless browser coverage around the principal preparation/job-result safety boundaries, and PR #189 deploys the browser-safe provider-authority preflight plus its create-time locked recheck. PR #188 closed the dependency/action/CD-observability maintenance batch without reopening the bounded production canary.
...
- Studio Platform CD is not generally broken: migration-changing pushes intentionally suppress automatic API deployment, and worker deployment is intentionally manual-only. PR #188 makes component selection, skip reasons, and final results explicit; post-merge run `30177718793` subsequently selected and deployed both source-changed web/API components, reported their successful results, and skipped the worker with reason `manual_only`.
...
| Studio source breadth | **about 98%** | Core processing, safety, analytics, duplicate/provider-attempt authority, and the real provider/Google path are proven. Historical Drive catalog import/standardization, accepted-output reuse, and finer optional telemetry remain. |
| Studio UX readiness | **about 95%** | Picker roles, single-attempt local upload/completion, prerequisites, cleanup feedback, CSP behavior, one real completed result, and browser-safe provider uncertainty have authenticated or deployed evidence. Broader catalog-migration UX and optional polish remain. |
| Studio production evidence | **about 99%** | Backup, migration, component identities, database compatibility, public health/security, clean smoke, one successful one-output canary, safe diagnostics/reconciliation, post-merge CI/CD, live result-status verification, and the deployed preflight-authority boundary are evidenced. The absent prior worker-image rollback candidate remains the main rollout limitation. |
| Studio combined v1 readiness | **about 96% deployed** | The core production processing path is live, bounded-canary proven, stabilized through Gate 7, and covered by authenticated CI. Remaining selected-scope work is led by the one-time catalog migration/standardization and its explicit duplicate/conflict UX; rollback-candidate limitations and the non-core roadmap remain. |

The working branch candidate is estimated at about **99% Studio source breadth**, **97% Studio UX readiness**, and **97% combined v1 readiness** before deployment. These implementation-only estimates do not change the current deployed snapshot or its **99% production evidence**; PR CI, migration `0016_transcript_catalog_entries`, component deployment, authenticated dry-run, and separately authorized apply evidence remain outstanding.
...
1. Resume `PWA-FRONTEND-MODULARIZATION-03`: extract bounded preparation composer/readiness behavior, then split `App.test.tsx` by the same domain boundaries.
2. Modularize `apps/studio-api/studio_api/main.py` into domain routers/response models, followed by a fixture-preserving split of `tests/test_studio_api_core.py`.
3. Simplify the 619-line `docs/ai-coding-workflow.md` in a dedicated documentation task; keep `AGENTS.md` as the lightweight router and avoid duplicating product/CI contracts.
...
- `main` and `origin/main` were synchronized at `95d3210` after PR #189. The active `codex/pwa-transcript-catalog-migration` branch starts from that exact revision.
- The active branch is a focused 15-commit catalog-migration source candidate. The final local pre-PR gate passed `788` portable Python tests with `6` skipped, all `285` frontend unit/component tests, ESLint, TypeScript, production build, lightweight repository checks, and `git diff --check`; Playwright discovery lists `9` authenticated scenarios. Earlier focused provider-boundary coverage passed `56` tests. The PostgreSQL/Redis-backed API and authenticated Chromium scenarios remain GitHub CI authority and have not been replaced by local SQLite evidence.
- Production rollout remains not-run for this candidate. Required evidence is green PR CI, tagged pre-migration backup, manual database migration to `0016_transcript_catalog_entries`, verified web/API deployment identities and health, an authenticated approved-folder dry-run canary, and a separately authorized single apply. No catalog Google mutation or production database change is implied by source readiness.
...
- Gates 4–7 remain complete. Authenticated production verification at `39aaaff` established the completed canary source as `Статус обработки: Завершена` with no raw `Статус файла: queued`; PR #187 subsequently deployed the expanded browser E2E source baseline at `7362a0c` without changing API or worker behavior. PR #188 deployed only the web component at `0b23320` and closed the dependency/action/CD-observability maintenance lane. PR #189 then deployed the changed web/API source at `95d3210` and closed `PWA-PREFLIGHT-UNCERTAINTY-01`. The active product item is `PWA-TRANSCRIPT-CATALOG-MIGRATION-01`.
