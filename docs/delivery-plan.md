...
- 👉 **PWA-SCOPE-ENTRY-01 — Align Google Colab vs Studio PWA scope, entry docs, workflow rules, and delivery state** — active/in PR: documentation-only alignment after PR #108. Explicit non-goals: no code changes, no new docs files, no worker/provider execution, no migrations, no CI/CD workflow changes, no Docker/runtime/deploy changes, no production rollout, and no secrets.
...
The active recommended item is **PWA-SCOPE-ENTRY-01 — Align Google Colab vs Studio PWA scope, entry docs, workflow rules, and delivery state**. It is a focused documentation-only PR that aligns `docs/project-spec.md`, `docs/delivery-plan.md`, `README.md`, `AGENTS.md`, and workflow/safety docs after PR #108.

Explicit non-goals: no application code changes; no new documentation files; no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Drive download/export; no source byte access; no Google Docs output creation; no output persistence; no manifest mutation; no job claiming/leasing implementation; no retry runner/background tasks; no public API endpoint; no backend schema changes or Alembic migrations; no CI/CD workflow changes; no Docker/Compose/runtime changes; no production deployment; no automatic migration rollout; no secrets, tokens, environment values, or file-mounted secret contents.
