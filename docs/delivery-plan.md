...
`apps/studio/` static mode remains demo-only and does not call `/api`. Platform mode now has Projects/source UI for the already-live projects and sources backends: project list/create/edit/archive, selected output Drive folder metadata, source listing/deletion, Google Drive source metadata creation from verified safe backend metadata, temporary local-upload intake, and Google Drive connection status/start/disconnect UI on account settings. PWA-GOOGLE-04A and PWA-GOOGLE-04B are source-done/merged on main. PWA-JOBS-01A is source-done/merged on main for persisted transcription job/job-source records plus authenticated create/list/detail/cancel APIs for jobs from existing project sources. PWA-JOBS-01B is source-done/merged on main for platform-mode per-project UI to create, list, inspect, and cancel queued/cancelled job records from existing uploaded sources. PWA-JOBS-01C is source-done/merged on main for optional active BYOK provider credential selection for queued job creation only. PWA-JOBS-01D is source-done/merged on main via PR #103 for record-only job lifecycle/status copy and readiness checklist before worker execution. PWA-JOBS-02-PREP is source-done/merged via PR #104. PWA-JOBS-02A is source-done/merged on main via PR #105. PWA-JOBS-02B is source-done/merged on main via PR #106. The current recommended item is PWA-JOBS-02C, a docs/design slice defining future server-side claim/lease semantics before any worker implementation. Provider execution, Drive picker, recursive browsing, Drive search, Google Docs output, processing, queues/workers, output persistence, sharing, production deployment, automatic migration rollout, and manifest mutation remain deferred. PWA-GOOGLE-01B is source-done/merged for runtime config wiring, but production rollout/live enablement is not claimed without operator evidence.
...
The active recommended item is **PWA-JOBS-02C — Studio job claim/lease contract before worker implementation**. It is a focused docs/design slice that defines the future server-side exclusive claim and bounded lease contract needed before any real worker can safely process queued Studio transcription jobs.

PWA-JOBS-02B is source-done/merged on main via PR #106. PWA-JOBS-02C must preserve the existing public API contract and keep Studio jobs record-only. It should document claim/lease concepts, ownership boundaries, idempotency, cancellation interaction, retry/recovery expectations, safe metadata boundaries, and the schema/runtime blockers for a later implementation.

Explicit non-goals: no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Drive download/export; no source byte access; no Google Docs output creation; no output persistence; no manifest mutation; no actual job claiming or lease acquisition/renewal/release; no retry runner or background tasks; no backend schema changes or Alembic migrations; no CI/CD changes; no Docker/Compose/runtime changes; no production deployment; no automatic migration rollout; no secrets, tokens, environment values, or file-mounted secret contents.
