...
- 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01` — Build the separately initiated one-time Google Docs catalog migration: browser-safe dry-run, explicit idempotent apply, eligible in-place `transcript_doc_v1.2` standardization, minimal durable Studio catalog metadata, and explicit duplicate/conflict outcomes. It must not become continuous Drive synchronization, import transcript/document bodies into Studio, or call a transcription provider.
...
3. 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01` — one-time dry-run/apply import and eligible in-place `transcript_doc_v1.2` standardization for the approved existing Google Docs collection, with minimal durable metadata and explicit duplicate/conflict outcomes.
