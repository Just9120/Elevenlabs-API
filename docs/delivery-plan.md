...
- ✅ `CI-MAINTENANCE-BATCH-01` — PR #188 merged at current `main` revision `0b23320`. The batch upgrades repository-owned GitHub Actions runtimes, adds explicit component-CD selection/result summaries, remediates the Node advisory graph to zero reported vulnerabilities, enforces the supported Node range, and hardens the read-only dependency-audit workflow without changing product behavior or deployment selection rules.
...
- 👉 `PWA-PREFLIGHT-UNCERTAINTY-01` — Source-complete on `codex/pwa-preflight-uncertainty`: owner-scoped in-flight/unresolved provider authority is projected into a browser-safe per-source preflight outcome, batch creation rechecks it under catalog source locks before creating rows, and the PWA blocks confirmation/reprocessing with bounded copy. A create-time race clears the stale preview without an automatic retry. The final worker provider-call guard is unchanged. PR CI, service-backed PostgreSQL coverage, and authenticated Playwright remain the acceptance gates.
...
- Studio has broad source-level implementation at merged `main` revision `0b23320`. Gates 4–7 remain complete: the real one-output ElevenLabs-to-Google-Docs path is production-proven, the result-status defect is deployed and authenticated-live verified, and PR #187 supplies repeatable secretless browser coverage around the principal preparation/job-result safety boundaries. PR #188 closed the dependency/action/CD-observability maintenance batch without changing product readiness or reopening the bounded production canary.
...
- `main` and `origin/main` were synchronized at `0b23320` after PR #188. The active `codex/pwa-preflight-uncertainty` branch starts from that exact revision and is fourteen focused commits ahead at the pre-PR checkpoint.
...
- The active preflight-authority batch passes the full locally applicable gate after final review fixes: `727 passed, 6 skipped` portable Python, all 279 Studio Vitest tests, ESLint, TypeScript, production Vite/PWA build, Playwright discovery with eight authenticated scenarios, lightweight repository checks, and `git diff --check`. Focused backend authority/worker-guard coverage passes locally (`64 passed`). The complete `main...HEAD` diff and commit series were reviewed without finding scope drift, secret material, an automatic provider retry, partial mixed-batch creation, or a weakened final worker guard. The new PostgreSQL API scenarios compile and collect, but their execution plus the existing authenticated Chromium suite remain GitHub CI acceptance evidence.
