...
- 👉 `PWA-PROCESSING-ROLLOUT-01A / Gate 4 closure` — Active branch `codex/pwa-production-rollout`. Run one clean authenticated public smoke against the deployed upload-recovery behavior: open both Picker roles, perform one local upload without a manual duplicate retry, confirm completion, verify the required credential/folder/source prerequisites, and observe the asynchronous cleanup message after source removal.
- ⏸ `PWA-PROCESSING-ROLLOUT-01A / Gates 5–6` — Worker deployment and the one-output canary remain blocked until the clean Gate 4 smoke passes. Worker-status run `30004841628` still reports the worker exited with `exit_code=0` and gracefully drained; its image identity remains unknown.
...
- Studio has broad source-level implementation at merged and deployed `main` revision `900bf5b`. The dominant remaining blocker is a clean authenticated public boundary followed by controlled worker/canary evidence, not missing core ElevenLabs processing code.
...
| Studio UX readiness | **about 83%** | The walkthrough fixes plus local-upload recovery and clearer cleanup behavior are merged, CI-validated, and deployed. A clean authenticated public re-smoke is still required. |
| Studio production evidence | **about 73%** | Backup, migration, current API/web images, database, services, public health, and the deployed upload-recovery boundary are evidenced. A clean post-fix smoke, current worker identity/deploy, and the one-output canary are not. |
| Studio combined v1 readiness | **about 73% deployed** | Upload reliability is deployed, but production readiness cannot jump materially until Gates 4–6 produce factual evidence. |
...
Current Gate 4 evidence is partial, so the exit is not met. The operator corrected R2 CORS and bucket-scoped credentials without exposing them, and the upload-recovery fixes are now deployed. The earlier smoke still cannot be used as the clean acceptance run because it included an ambiguous first attempt, a manual second attempt, duplicate objects, and a `403` completion response. One clean authenticated re-smoke is required. Physical cleanup of deleted/expired local objects also remains pending while the worker is intentionally stopped.
