...
- 👉 `PWA-PREFLIGHT-UNCERTAINTY-01` — Active: project unresolved/in-flight provider authority into a browser-safe preflight rejection without weakening the final provider-call guard. The accepted-output checkbox must not override unresolved provider outcomes.
...
- `main` and `origin/main` were synchronized at `0b23320` after PR #188. The active `codex/pwa-preflight-uncertainty` branch starts from that exact revision.
