...
- 👉 **PWA-JOBS-03D — Ephemeral single-source byte materialization boundary** — active/in PR: adds an internal context-managed boundary that materializes exactly one verified job source into bounded ephemeral temporary storage for future provider execution. It still adds no worker, queue consumer, provider execution, provider credential use, Google Docs output, output persistence, completed transition, manifest mutation, public endpoint, frontend, migration, CI/CD/runtime/deploy change, production rollout, VPS access, or secrets change.
...
The active recommended item is **PWA-JOBS-03D — Ephemeral single-source byte materialization boundary**. It is an internal source-only boundary for jobs already in `processing`: exact lease owner/generation and active lease are required; cancellation, missing/archived project state, relation/source mutations, local object failures, and Drive content failures block materialization. It reuses the PWA-JOBS-03C availability verification immediately before byte access, copies one source into bounded ephemeral temporary storage, revalidates identity after copy, and does not authorize provider execution or output creation.

Explicit non-goals: no new documentation files; no worker implementation; no queue consumer; no provider API calls; no provider credential decryption/use at runtime; no Google Docs API calls or output creation; no output persistence; no manifest mutation implementation; no retry runner/background tasks; no public API endpoint; no new job status values beyond existing `queued`/`processing`/terminal states; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no VPS access; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, or presigned URLs.
