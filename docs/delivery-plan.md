...
| `COLAB-BATCH-01` | **73,9% (`17/23`)** | **73,9% (`17/23`)** | 🟦 IN PROGRESS; functional CI ✅, aggregate dependency/security CI ❌; `SPEC ✅ CODE ◐ TEST ◐ CI ◐ DEPLOY ◐ LIVE ◐`. |
...
| `PWA-REALTIME-01` | **83,3% (`5/6`)** | **83,3% (`5/6`)** | 🟦 IN PROGRESS; capture fix deployed и Chrome LIVE подтверждён, но full microphone/display/mixed matrix для `PR-06` остаётся неполной; functional CI ✅, aggregate dependency/security CI ❌: `SPEC ✅ CODE ✅ TEST ✅ CI ◐ DEPLOY ✅ LIVE ◐`. |
...
- Scheduled Dependency audit run `31996248930` на baseline `f3a3f33` завершился failure; его remediation является Current Goal.
