...
- 🧪 `PWA-MEDIA-PREPARATION-01A` — The current branch adds the first bounded media-preparation slice: server-validated video sources are converted to AAC/M4A by the worker before ElevenLabs, with no-shell `ffmpeg`, timeout/output limits, temporary-artifact cleanup, normalized pre-provider failures, and lifecycle revalidation after preparation. Focused tests (`62 passed`), the portable suite (`670 passed, 6 skipped`), lightweight repository checks, and diff checks pass locally. Automatic long-media size/duration split and merge remain the next separate slice; Docker image build, service-backed CI, and merge evidence are pending.
...
| Studio source breadth | **about 88%** | Major ElevenLabs short-media workflow, lifecycle safety, PWA shell, operations, test foundations, and the typed language/diarization path exist on the current branch. |
...
| Studio combined v1 readiness | **about 75% ±5** | Weighted planning estimate across product, parity, quality, operations, and production evidence. |
