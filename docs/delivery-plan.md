...
- 👉 **STUDIO-CD-STDIN-01 — Prevent deploy-script stdin consumption and false-success CD runs** — active focused CD correctness item: materialize the trusted deploy script before execution and detach non-interactive Alembic Compose checks from stdin so automatic CD cannot report success before forced recreation, running-image identity verification, and health pass.
- 📋 **PWA-OUTPUT-01A — Internal Google Docs single-transcript write boundary** — next product item after this CD follow-up.
...
The active recommended item is **STUDIO-CD-STDIN-01 — Prevent deploy-script stdin consumption and false-success CD runs**. It is an operational CD correctness fix for `.github/workflows/studio-platform-cd.yml` and `scripts/deploy_studio_platform_component.sh`: automatic CD must materialize the trusted `origin/main` deploy script into a temporary file before executing it, and API Alembic Compose checks must be detached from stdin so child commands cannot consume the remaining Bash program before forced recreation, running-image identity verification, health polling, and the component success marker.

After STUDIO-CD-STDIN-01, the next product item remains **PWA-OUTPUT-01A — Internal Google Docs single-transcript write boundary**.

Explicit non-goals for STUDIO-CD-STDIN-01: no product behavior change; no provider execution; no Google Docs output; no worker or queue; no public API or frontend change; no models or migrations; no PostgreSQL, Redis, volume, backup, restore, nginx, runtime `.env`, Repository Secret/Variable, SSH access, Docker base image, dependency, retry, rollback, cleanup, hardening, or VPS mutation scope.
