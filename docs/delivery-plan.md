...
- 👉 **PWA-OUTPUT-02A — Browser-safe Studio job output API** — active implementation item.
...
The active recommended item is **PWA-OUTPUT-02A — Browser-safe Studio job output API**. This source-only implementation item adds the explicit authenticated, owner-scoped, read-only `GET /api/jobs/{job_id}/outputs` endpoint for persisted output discovery with a closed allowlist serializer, parsed Google web-view URL validation, partial-output semantics, deterministic ordering, focused tests, and narrow documentation updates. It must not add frontend output rendering, processing behavior, worker changes, Compose changes, deployment, secrets, or production rollout.
