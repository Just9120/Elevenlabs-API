...
- 👉 **PWA-BATCH-COMPOSER-01 — Combine preparation UX and create source-to-folder rows** — current in-PR frontend/tests/docs item: normal platform project workspace uses `Обзор` plus combined `Подготовка`, source intake and job history live with the ordered source→output-folder composer, and normal platform creation submits atomically through `/jobs/batch`. Runtime smoke, worker startup, production processing, deploy, provider calls, Google Docs creation, rollback, cleanup, and manifest parity remain separate.
...
- 👉 PWA-BATCH-COMPOSER-01 — Combine preparation UX and create source-to-folder rows — in PR #146, not done/merged until review and merge complete.
