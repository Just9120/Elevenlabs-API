...
- 👉 **PWA-JOBS-01C — Studio job BYOK credential selection UI** — active frontend/docs item: add optional active BYOK provider credential selection to queued job creation only. It uses safe credential metadata from `GET /api/credentials` and sends only the selected credential identity, or no credential, to the existing job create API. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no backend schema/API changes unless a frontend-blocking contract bug is found, no production deployment, and no automatic migration rollout.
...
The active Studio frontend/docs item is PWA-JOBS-01C: add the smallest platform-mode UI addition for optional active BYOK provider credential selection during queued job creation. It uses existing safe credential metadata and the existing job create contract only. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no backend schema/API changes unless a frontend-blocking contract bug is found, no production deployment, and no automatic migration rollout.
