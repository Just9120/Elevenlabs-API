...
Continue local PWA stability hardening with a focused audit of the Settings Google connection boundary: connection GET, OAuth start redirect capability, reconnect, and disconnect. Verify runtime DTO/capability validation, request deadlines, synchronous duplicate prevention, remount ownership, redirect safety, authoritative reconciliation of ambiguous disconnect, and safe error rendering. Keep maintenance OAuth, Google Picker composer flows, backend OAuth rules, API/schema, deployment, and production state outside this item unless shared evidence requires a narrowly documented boundary.
