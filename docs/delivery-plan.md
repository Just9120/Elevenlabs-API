...
- 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Active operator-gated item. One tagged pre-migration backup is authorized but has not started. Read-only preflight run `30250456568` reached the VPS through the approved GitHub SSH boundary, then stopped before its service/revision table because the deploy user cannot directly read a protected operator-owned source-storage secret file. Required sequence remains an approved privileged backup boundary, verified backup, separately authorized migration to `0016_transcript_catalog_entries`, intended API deployment/identity/health, authenticated approved-folder dry-run, and separately authorized apply.
...
| Selected Studio v1 source/CI | **99% authoritative** | Exact-revision corrective CI passed. On the same ten-epic denominator, auth is `4/4` and frontend UX/modularization is `3.5/4`: `39.5/40 = 98.75%`, rounded to 99%. |
...
1. Resolve the approved privileged execution path for protected operator-owned backup secrets, then create and verify the already authorized tagged pre-migration backup without weakening secret-file permissions.
...
- Catalog production rollout remains paused before the authorized backup because the repository has no GitHub backup workflow and the deploy user cannot directly read protected operator-owned backup/source secret files. Do not change secret ownership or mode to bypass this boundary; use an explicitly approved privileged operator path, then continue with database `0016`, intended API deployment identity/health, an authenticated approved-folder dry-run, and separate authorization for one bounded apply.
