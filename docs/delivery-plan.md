...
- 👉 `PWA-PROCESSING-ROLLOUT-01A / Gate 4 host boundary` — Active branch `codex/pwa-production-rollout`. The clean authenticated functional smoke passed both Picker roles, one single-attempt local upload/completion, required credential/folder/source prerequisites, and source-removal cleanup messaging. The remaining Gate 4 blocker is the unapplied public-host security-header policy: TLS/public routing are healthy, but the live PWA and API responses omit the committed CSP, HSTS, MIME-sniffing, referrer, permissions, and framing headers.
- ⏸ `PWA-PROCESSING-ROLLOUT-01A / Gates 5–6` — Worker deployment and the one-output canary remain blocked until the host nginx policy is applied, `nginx -t` passes, and the public headers are re-verified. Worker-status run `30004841628` still reports the worker exited with `exit_code=0` and gracefully drained; its image identity remains unknown.
...
- Studio has broad source-level implementation at merged and deployed `main` revision `900bf5b`. The dominant remaining blockers are applying the committed public-host header policy and then collecting controlled worker/canary evidence, not missing core ElevenLabs processing code.
...
- Live TLS/public routing are healthy for both the PWA and API (`200` over TLS 1.3 with a valid host certificate), but neither response includes the committed browser security-header policy. Standard component CD intentionally does not apply host nginx, so Gate 4 now stops at an explicit operator host-config rollout and public re-verification.
...
| Studio UX readiness | **about 88%** | Both Picker roles, single-attempt local upload/completion, prerequisites, and asynchronous source-removal feedback now have clean authenticated public evidence. Processing-result UX still awaits the controlled canary. |
| Studio production evidence | **about 78%** | Backup, migration, current API/web images, database, services, public health, and the clean functional Gate 4 smoke are evidenced. The unapplied host-header policy, current worker identity/deploy, and the one-output canary remain. |
| Studio combined v1 readiness | **about 77% deployed** | The browser intake boundary is proven, but the public security policy must be applied before the worker and canary gates may start. |
...
Current Gate 4 evidence is partial, so the exit is not met. The clean authenticated smoke passed both Picker roles, one single-attempt local upload and completion, one writable output folder, one active ElevenLabs credential, a valid Google connection, and the source-removal boundary. The removed local source was immediately excluded from preparation and the PWA reported queued background cleanup without waiting for the retention deadline; physical deletion remains pending while the worker is intentionally stopped. Independent public checks returned `200` over TLS 1.3 with a valid host certificate for both the PWA and API, but neither response contained the required security headers. Apply the committed host nginx configuration as an explicit operator action, require `nginx -t`, reload nginx, and re-verify both routes before Gate 5.
...
- Independent public checks returned `200` over TLS 1.3 with a valid `studio.librechat.online` certificate for both `/` and `/api/healthz`. Both responses omitted the committed CSP, HSTS, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, and `X-Frame-Options` headers. Host nginx rollout plus `nginx -t` and public re-verification are the remaining Gate 4 blocker.
- Current worker rollout and the one-output canary remain unproven and unauthorized while Gate 4 is open.
