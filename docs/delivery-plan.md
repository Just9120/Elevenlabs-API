...
- 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production dry-run` — Ready for an explicitly approved-folder dry-run. Verified snapshot `15ca68cc3e4f` protected the separately authorized migration from `0015_user_source_retention` to `0016_transcript_catalog_entries`; the post-check found the new catalog table with zero rows. API-only deployment runs `30263481907` and `30287692026` verified revision equality, intended image identity, configuration-consuming API recreation, and health at `main@93de8cf`. The edge-policy repair and Picker opening are verified; dry-run and one separately authorized bounded apply still remain.
...
- Repository head, production PostgreSQL, and the deployed API agree on `0016_transcript_catalog_entries`. The merged public edge repair is also live: local and public TLS expose only the production origin, without path/query data, public web/API health remains green, and authenticated Chrome opens the Picker list without the prior invalid-key failure. Catalog production proof still requires approved-folder dry-run evidence plus a separately authorized apply.
...
| Catalog migration production rollout | **60% (`3/5`)** | The verified backup plus `0016` complete the schema/config prerequisite, and the intended API deployment/identity/health gate is complete. Picker loading is unblocked, but approved-folder dry-run and separately authorized apply remain. |
...
1. Run an explicitly approved-folder catalog dry-run, review only safe aggregate results, and require separate authorization for one bounded apply.
...
- Authenticated Chrome opened the production Google Picker list without the prior invalid-key failure under the live `origin` policy. This proves Picker loading only; no approved-folder selection, safe aggregate dry-run result, or apply evidence was provided. The catalog rollout may proceed only with an explicitly approved-folder dry-run, and apply remains a separate authorization. Do not infer authorization for apply from the backup, migration, deployment, configuration repair, edge-policy repair, Picker opening, or dry-run.
