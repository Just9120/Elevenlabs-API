...
- 👉 **PWA-GOOGLE-03B — Studio Drive metadata verification frontend** — active frontend/docs item: platform-mode Sources UI verifies one explicit Drive file/folder ID, previews safe backend metadata, and creates a Google Drive source record from verified metadata.
...
The active Studio frontend/docs item is PWA-GOOGLE-03B: platform-mode Sources UI should call the existing backend metadata endpoint for one explicitly supplied Google Drive file/folder ID, preview only safe normalized metadata, and create a Google Drive source record from that verified metadata through the existing CSRF-protected source endpoint. Explicit non-goals: no Drive picker, no Drive listing/browsing UI, no Google Docs creation, no provider execution, no jobs/queues/workers, no migration, no production deployment, and no manifest mutation.
