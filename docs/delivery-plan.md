...
- 👉 **PWA-JOBS-01A — Studio transcription job backend foundation** — active backend/docs item: add job/job-source persistence and authenticated create/list/detail/cancel APIs for jobs from existing project sources only. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no frontend UI, no production deployment, and no automatic migration rollout.
...
`apps/studio/` static mode remains demo-only and does not call `/api`. Platform mode now has Projects/source UI for the already-live projects and sources backends: project list/create/edit/archive, selected output Drive folder metadata, source listing/deletion, Google Drive source metadata creation from verified safe backend metadata, temporary local-upload intake, and Google Drive connection status/start/disconnect UI on account settings. PWA-GOOGLE-04A and PWA-GOOGLE-04B are source-done/merged on main. The current backend/docs item is PWA-JOBS-01A: add persisted transcription job/job-source records plus authenticated create/list/detail/cancel APIs for jobs from existing project sources. Provider execution, Drive picker, recursive browsing, Drive search, Google Docs output, processing, queues/workers, output persistence, sharing, frontend job UI, production deployment, automatic migration rollout, and manifest mutation remain deferred. PWA-GOOGLE-01B is source-done/merged for runtime config wiring, but production rollout/live enablement is not claimed without operator evidence.
...
The active Studio backend/docs item is PWA-JOBS-01A: add the smallest persisted transcription job foundation for existing project sources. It adds job/job-source database state and authenticated create/list/detail/cancel APIs only. Explicit non-goals: no worker, no provider execution, no queue consumer, no Google Docs creation, no output persistence, no manifest mutation, no frontend UI, no production deployment, and no automatic migration rollout.
