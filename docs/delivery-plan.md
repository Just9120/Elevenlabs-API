...
- 👉 **PWA-OUTPUT-02B — Studio job output links UI** — active implementation item.
...
The active recommended item is **PWA-OUTPUT-02B — Studio job output links UI**. This frontend-focused item renders persisted output metadata and available Google Docs/Drive links only after the user opens a job, keeps job lifecycle status separate from output availability, preserves static-mode zero-`/api` behavior, and avoids polling or production-runtime claims. It must not add backend routes, processing behavior, worker changes, Compose changes, deployment, secrets, migrations, or production rollout.
