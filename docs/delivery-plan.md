...
- Studio PWA production processing is not claimed: current jobs have internal processing/provider/output boundary slices through a synchronous single-job orchestrator, safe output persistence, and fenced completion, but no worker, queue consumer, public processing pipeline, production processing pipeline, or manifest mutation.
...
- 👉 **PWA-PIPELINE-01A — Internal synchronous single-job orchestrator** — active implementation item only; add the internal server-only synchronous orchestrator that composes the existing boundaries for one already-leased job according to the PWA-PIPELINE-01-PREP contract.
...
The active recommended item is **PWA-PIPELINE-01A — Internal synchronous single-job orchestrator**. It implements only the internal synchronous function that composes existing job lifecycle, provider, Google Docs creation, output persistence, and fenced completion boundaries for one already-leased job. It must not add a worker, public processing API, output browser API, manifest behavior, automatic retry/reconciliation, runtime/deploy changes, production migration execution, or production-live processing claim.
