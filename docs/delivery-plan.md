...
- 👉 **PWA-PIPELINE-01B — Internal one-shot claim-and-process boundary** — active implementation item only; add the internal server-only synchronous boundary that claims one explicitly identified queued job, commits the lease, and invokes the existing synchronous orchestrator.
...
The active recommended item is **PWA-PIPELINE-01B — Internal one-shot claim-and-process boundary**. It implements only the internal synchronous function that claims one explicitly identified queued job, commits the acquired lease, and invokes the existing synchronous orchestrator with the committed owner/generation. It must not add job discovery, polling, a worker, public processing API, output browser API, manifest behavior, automatic retry/reconciliation, runtime/deploy changes, production migration execution, or production-live processing claim.

No worker/runtime follow-up is newly approved by this update.
