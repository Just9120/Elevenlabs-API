...
1. Finish the local recursive-maintenance batch, run the full applicable gate, review the complete `main...HEAD` diff, publish one PR, and merge only after exact-head CI is green.
...
- The current local candidate implements the subsequent recursive product decision: separate encrypted maintenance grant, exact same-account/scope validation, server-only refresh, recursive bounded traversal, fresh folder-only dry-run/apply scans, per-document isolation, and separate PWA consent/state. The ten source tasks and focused backend/frontend/config gates completed so far pass; the final full local regression gate is still pending in this batch.
