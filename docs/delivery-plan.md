...
- 👉 `PWA-E2E-FOUNDATION-01B` — Extend authenticated browser coverage across preparation and job-result behavior with controlled provider/Google/S3 boundaries. Keep real provider/Google canaries separate and preserve the current secretless CI contract.
...
- Studio has broad source-level implementation at merged `main` revision `39aaaff`. Gates 4–7 are complete: the real one-output ElevenLabs-to-Google-Docs path is production-proven, and the result-status defect is deployed and authenticated-live verified. The next active lane expands repeatable browser coverage rather than reopening the bounded production canary.
...
| Studio UX readiness | **about 94%** | Picker roles, single-attempt local upload/completion, prerequisites, cleanup feedback, CSP behavior, and one real completed result have authenticated public evidence. The corrected per-source completed status is deployed and live-verified; broader preparation/result browser coverage remains the active lane. |
...
| Studio combined v1 readiness | **about 95% deployed** | The core production processing path is live, bounded-canary proven, and stabilized through Gate 7. Remaining work is broader repeatable E2E coverage and the explicitly listed non-core roadmap. |
...
- `main` and `origin/main` are `39aaaff` (PR #186). The active `codex/pwa-e2e-foundation` branch starts from that exact revision.
- PR checks passed, including service-backed `browser-e2e` job `89569670478`. Post-merge repository CI run `30120922527` and Studio run `30120922513` passed; the latter executed both `studio` and `browser-e2e` jobs.
- Component CD run `30120922496` detected only the web change. `deploy-web` fast-forwarded the production checkout `900bf5b → 39aaaff`, rebuilt the web image, verified running/built image identity equality, passed localhost `/healthz`, and emitted `STUDIO_PLATFORM_WEB_DEPLOY_OK`. `deploy-api` and manual-only `deploy-worker` were correctly skipped.
...
- Gates 4–7 are complete. Authenticated production verification of the deployed web at `39aaaff` shows the completed canary source as `Статус обработки: Завершена` and no raw `Статус файла: queued`. The next active item is `PWA-E2E-FOUNDATION-01B`.
