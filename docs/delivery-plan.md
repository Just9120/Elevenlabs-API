...
- 👉 **PWA-AUTH-OAUTH-UX-01 — Smooth session bootstrap and return from Google OAuth** — current focused frontend/backend UX item: fix two manually observed production defects where the Login form flashes while `/api/auth/session` is still being checked and Google OAuth callback leaves the browser on an API status/JSON page instead of returning to the PWA.
