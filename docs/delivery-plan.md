...
- 👉 `PWA-TRANSCRIPT-MAINTENANCE-RECURSIVE-02 / local source candidate` — Standardization and `Манифест Studio` now accept only their own selected root `folder_id`, recursively rescan descendants, skip target-state documents, isolate safe per-document failures, and use a separately consented/stored server-only maintenance OAuth grant. The maintenance subject must match the primary Picker account; its tokens never reach the browser or worker. Source tasks and targeted gates pass locally; exact-head CI, migration `0017_google_maintenance_oauth`, runtime secret/config rollout, deployed identity, authenticated recursive dry-runs, and separately approved applies remain.
- 📋 `PWA-TRANSCRIPT-MAINTENANCE-RECURSIVE-02 / rollout` — After merge: prepare a new tagged backup, apply and verify `0017`, configure the separate maintenance OAuth client/secret on the VPS, deploy exact web/API images, complete same-account consent, then run one safe dry-run per operation. Apply requires a later explicit decision per operation.
...
- Studio has broad source-level implementation at merged `main@5ab3b5f`. Exact-main repository run `30335920148` and Studio/browser run `30335920193` are green for that revision.
...
- Merge-triggered Studio Platform CD run `30335920233` selected and deployed web and API from exact `main@5ab3b5f`, verified the API database/image revision boundary and running image identity, and skipped the manual-only worker.
...
- Merged main, production PostgreSQL, and the deployed API agree on `0016_transcript_catalog_entries`. The merged public edge repair is live, and PR #195's explicit-document split is deployed. Neither fact proves the newly selected recursive behavior.
- The current local branch replaces browser-supplied document selection with bounded recursive server traversal plus a separate server-only maintenance grant and repository migration `0017_google_maintenance_oauth`. It is neither merged nor deployed; no recursive dry-run or apply has been attributed to it.
...
| Selected Studio v1 source/CI | **98% local candidate (`39/40`)** | On the existing ten-epic/four-gate denominator, the affected maintenance epic has current contract, source, and targeted evidence; exact-head PR CI is the one missing gate. Green exact-main CI at `5ab3b5f` validates the superseded explicit-document implementation, not this branch. |
| Recursive transcript-maintenance source tasks | **100% (`10/10`)** | Separate OAuth boundary/storage/config/refresh/lifecycle, canonical Docs parsing, recursive scan/preview, folder-only routes, per-document isolation, and recursive UI/consent are implemented. This is source completion, not rollout completion. |
...
| Recursive transcript-maintenance rollout | **0% (`0/4`)** | Required gates are: `0017` plus maintenance runtime config, exact web/API deployment identity and health, one meaningful recursive dry-run per operation, and one separately authorized bounded apply per operation. None belongs to this branch yet. |

The scoring method and partial-gate rule are in `docs/audits/repository-audit-2026-07-26.md`. Exact-main `5ab3b5f` checks remain authoritative for merged source, while current-branch targeted evidence covers only the recursive candidate. Documentation, local code, and prior zero-item requests cannot increase production readiness.

## Product roadmap after production proof

Order work so each capability inherits a known source and production baseline:

1. Publish the completed local recursive-maintenance batch as one PR, inspect the exact-head checks, and merge only after CI is green.
2. Prepare a new tagged PostgreSQL backup, configure the separate maintenance OAuth client/secret without exposing values, apply and verify `0017_google_maintenance_oauth`, then deploy only exact web/API components under the CI/CD safety contract.
3. Verify exact checkout/image identities, public and localhost health, production revision `0017`, primary Picker readiness, and same-account maintenance consent. Worker rollout is not part of this maintenance release.
4. In the user's authenticated Chrome, select one bounded canary root per panel and run one non-mutating recursive dry-run for standardization and one separate dry-run for `Манифест Studio`; review only safe aggregate results.
5. Require a separate explicit authorization for each bounded apply. Never use one operation's root, preview, confirmation, or result as authority for the other.
6. Run bounded production canaries for the remaining selected modes: auto-detect language, diarization, video preparation, long-media split/merge, and multi-file processing.
7. Confirm external consumers of the two deprecated compatibility APIs, then either remove them or record an explicit support/removal contract. Existing deprecation/successor headers do not prove that consumers have migrated.
8. Resume preparation-composer and API-router modularization in bounded behavior-preserving slices.
9. Add golden Colab/PWA fixtures for normalization, ordering, output shape, and failure semantics.
10. Validate claim/lease/heartbeat/recovery behavior under concurrency before increasing the production worker count.
...
- Local `main` and `origin/main` are synchronized at merge revision `5ab3b5f`. Merge-triggered CD run `30335920233` fetched that exact main revision, deployed web and API successfully, and skipped the manual-only worker.
- Exact-main repository CI run `30335920148` and Studio run `30335920193` passed at `5ab3b5f`, including the repository, PostgreSQL/Redis/Alembic, frontend, authenticated browser, image-build, protected-secret bootstrap, and Compose gates owned by those workflows.
...
- The current local candidate implements the subsequent recursive product decision: separate encrypted maintenance grant, exact same-account/scope validation, server-only refresh, recursive bounded traversal, fresh folder-only dry-run/apply scans, per-document isolation, separate PWA consent/state, and no remaining browser mode for selecting individual maintenance documents. The final local gate passed: lightweight checks and ESLint were clean, the portable Python suite reported `868 passed, 6 skipped`, all `312` Vitest tests across `28` files passed, the `126` tests covering the final UI cleanup passed again, TypeScript and the production build passed, Python compilation and diff checks passed, and Playwright discovered the existing `9` E2E scenarios. Exact-head Linux/Docker CI remains the source/CI gate not supplied by local evidence.
- Repository candidate head is `0017_google_maintenance_oauth`, while production remains operator-evidenced at `0016_transcript_catalog_entries`. The new runtime client ID/secret/scopes are not configured or deployed, no same-account maintenance consent exists in production, and no recursive dry-run/apply has run. Those are explicit rollout blockers, not source defects.
