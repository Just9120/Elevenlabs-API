...
- 👉 **PWA-JOBS-03B — Studio processing lifecycle, attempts, and cancellation-request foundation** — active/in PR: adds `processing` state-machine foundations, `attempt_count`, processing-time cancellation requests, safe terminal failure/cancellation transitions, processing lease renew/no-raw-release semantics, and explicit expired-lease recovery primitives only. It still adds no worker, queue consumer, provider execution, credential use, source access, Google Docs output, output persistence, completed transition, manifest mutation, scheduler/recovery runner, production rollout, VPS access, CI/CD change, Docker/runtime/deploy change, or secrets change. Migrations 0005/0006/0007 remain manual/operator-scoped for production.
...
The active recommended item is **PWA-JOBS-03B — Studio processing lifecycle, attempts, and cancellation-request foundation**. It is a source-only lifecycle foundation that adds processing status, attempt counting, request/acknowledge cancellation semantics, safe failure, processing lease renewal/no-raw-release behavior, and explicit expired-lease recovery primitives. It still does not add worker/provider/source/output/manifest/runtime/deploy/production/secrets work.

Explicit non-goals: no new documentation files; no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Drive download/export implementation; no source byte access; no Google Docs API calls or output creation; no output persistence; no manifest mutation implementation; no retry runner/background tasks; no public API endpoint; no new job status values; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no VPS access; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, or presigned URLs.
