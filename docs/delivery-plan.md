...
- ⛔ `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Blocked before approved-folder selection. Verified snapshot `15ca68cc3e4f` protected the separately authorized migration from `0015_user_source_retention` to `0016_transcript_catalog_entries`; the post-check found the new catalog table with zero rows. API-only deployment run `30263481907` then verified revision equality, recreated only the intended API, matched its running image identity, and passed localhost/public health at `main@93de8cf`. Authenticated Picker attempts in both the controlled browser and the operator's Google Chrome failed before selection; Chrome explicitly reported an invalid API key. Repair and revalidate the Picker runtime/Google Cloud credential boundary before retrying the dry-run. One bounded apply still requires separate explicit authorization.
...
1. Repair and revalidate the production Google Picker API key/project configuration, then run an authenticated approved-folder catalog dry-run and require separate authorization for one bounded apply.
