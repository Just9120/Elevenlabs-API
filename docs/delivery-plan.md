...
- 👉 **PWA-JOBS-03C — Studio processing-time source availability verification foundation** — active/in PR: adds an internal read-only verification boundary for `processing` jobs that checks Drive metadata and private S3/R2 object HEAD immediately before future source materialization. It still adds no worker, queue consumer, source byte download/materialization, provider execution, provider credential use, Google Docs output, output persistence, completed transition, manifest mutation, public endpoint, migration, CI/CD/runtime/deploy change, production rollout, VPS access, or secrets change.
...
The active recommended item is **PWA-JOBS-03C — Studio processing-time source availability verification foundation**. It is a server-side verification foundation for jobs already in `processing`: exact lease owner/generation and active lease are required; cancellation, missing/archived project state, missing sources, local expiry/object metadata failures, and Drive metadata failures block readiness. It performs read-only Google Drive metadata checks and private S3/R2 HEAD checks only; the result is an ephemeral safe summary, not permanent authorization for provider execution.

Explicit non-goals: no new documentation files; no worker implementation; no queue consumer; no provider API calls; no provider credential decryption/use at runtime; no Google Drive download/export byte access; no source byte access/materialization; no Google Docs API calls or output creation; no output persistence; no manifest mutation implementation; no retry runner/background tasks; no public API endpoint; no new job status values beyond existing `queued`/`processing`/terminal states; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no VPS access; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, or presigned URLs.
