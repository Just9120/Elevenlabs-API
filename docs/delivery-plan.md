...
- ✅ **PWA-JOBS-02B — Studio job processing preflight snapshot for existing record-only jobs** — source-done/merged on main via PR #106: added a small internal read-only backend preflight helper and focused tests for conservative existing job/source/output readiness metadata only. Explicit non-goals remain no worker/provider execution, queue consumer, credential decryption/use, Google Drive download/export, source byte access, Google Docs output, output persistence, manifest mutation, job claiming/leases, migrations, CI/CD, Docker/runtime/deploy, production rollout, or secrets handling changes.
- ✅ **PWA-JOBS-02C — Studio job claim/lease contract before worker implementation** — source-done/merged on main via PR #107: documented future server-side exclusive claim and bounded lease semantics before a real worker can safely process queued Studio transcription jobs. Explicit non-goals remain no worker/provider execution, queue consumer, credential decryption/use, Google Drive download/export, source byte access, Google Docs output, output persistence, manifest mutation, actual claiming/leasing, retry runner, backend schema changes/migrations, CI/CD, Docker/runtime/deploy, production rollout, or secrets handling changes.
- 👉 **PWA-JOBS-02D — Studio job claim-readiness plan helper for existing record-only jobs** — active/in PR: add a small internal, non-mutating helper that converts the existing read-only processing preflight snapshot into conservative future claim-readiness metadata. Explicit non-goals remain no actual job claiming/leasing, worker/provider execution, queue consumer, credential decryption/use, Google Drive download/export, source byte access, Google Docs output, output persistence, manifest mutation, retry runner/background tasks, public API endpoint, backend schema changes/migrations, CI/CD, Docker/runtime/deploy, production rollout, or secrets handling changes.
...
`apps/studio/` static mode remains demo-only and does not call `/api`. Platform mode now has Projects/source UI for the already-live projects and sources backends: project list/create/edit/archive, selected output Drive folder metadata, source listing/deletion, Google Drive source metadata creation from verified safe backend metadata, temporary local-upload intake, and Google Drive connection status/start/disconnect UI on account settings. PWA-GOOGLE-04A and PWA-GOOGLE-04B are source-done/merged on main. PWA-JOBS-01A is source-done/merged on main for persisted transcription job/job-source records plus authenticated create/list/detail/cancel APIs for jobs from existing project sources. PWA-JOBS-01B is source-done/merged on main for platform-mode per-project UI to create, list, inspect, and cancel queued/cancelled job records from existing uploaded sources. PWA-JOBS-01C is source-done/merged on main for optional active BYOK provider credential selection for queued job creation only. PWA-JOBS-01D is source-done/merged on main via PR #103 for record-only job lifecycle/status copy and readiness checklist before worker execution. PWA-JOBS-02-PREP is source-done/merged via PR #104. PWA-JOBS-02A is source-done/merged on main via PR #105. PWA-JOBS-02B is source-done/merged on main via PR #106. PWA-JOBS-02C is source-done/merged on main via PR #107. The current recommended item is PWA-JOBS-02D, a coding guardrail slice adding an internal non-mutating claim-readiness plan helper before any worker implementation. Provider execution, Drive picker, recursive browsing, Drive search, Google Docs output, processing, queues/workers, output persistence, sharing, production deployment, automatic migration rollout, and manifest mutation remain deferred. PWA-GOOGLE-01B is source-done/merged for runtime config wiring, but production rollout/live enablement is not claimed without operator evidence.
...
The active recommended item is **PWA-JOBS-02D — Studio job claim-readiness plan helper for existing record-only jobs**. It is a focused coding guardrail slice that adds a small internal, non-mutating helper to convert the existing read-only processing preflight snapshot into conservative future claim-readiness metadata.

PWA-JOBS-02C is source-done/merged on main via PR #107. PWA-JOBS-02D must preserve the existing public API contract and keep Studio jobs record-only. It should reuse the preflight helper, report only safe metadata, keep output-folder and provider-credential fields as signals only, and avoid inspecting or faking future claim/lease schema fields that do not exist yet.

Explicit non-goals: no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Drive download/export; no source byte access; no Google Docs output creation; no output persistence; no manifest mutation; no actual job claiming or lease acquisition/renewal/release; no retry runner or background tasks; no public API endpoint; no backend schema changes or Alembic migrations; no CI/CD changes; no Docker/Compose/runtime changes; no production deployment; no automatic migration rollout; no secrets, tokens, environment values, or file-mounted secret contents.
