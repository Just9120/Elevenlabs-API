...
- Migration `0018_job_part_progress` is additive but still stateful. Ordinary component CD must not apply it. The protected migration lane, exact API/worker deployment, and a bounded production UI canary remain required after merge.
...
| `PWA-JOB-PROGRESS-02` production rollout | **0% (`0/4`)** | Required gates are merged CI; `0018` plus API rollout; worker identity/health; real UI canary. None applies to the unmerged branch. |
...
`PWA-JOB-PROGRESS-02` acceptance checks:
...
6. Repository Alembic head is the single additive direct successor `0018_job_part_progress`; ordinary component CD does not apply it.
...
1. Merge and roll out `PWA-JOB-PROGRESS-02` through its stateful release gates.
...
- The progress branch is not pushed or reviewed and changes repository head from production `0017` to candidate `0018`.
...
- Branch: `codex/pwa-job-progress-02`, based on clean `main@bd8d513`; after the source implementation it is `0 behind / 3 ahead` before this documentation commit.
- Focused backend: `75 passed` across job progress, retry recovery, part schema, maintenance schema, and ElevenLabs execution.
- Focused frontend: `5 passed`; TypeScript build and targeted ESLint passed.
...
- Self-review: the package changes only job-result continuity, browser-safe progress projection, fenced integer part counters, the direct additive migration, and matching contracts/tests/docs. It does not implement realtime, change provider semantics, expose content, or mutate production.
