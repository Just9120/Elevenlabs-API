...
- 🧪 `PWA-PREFLIGHT-PROGRESS-01A` — The current branch adds a non-mutating server-authoritative batch preflight and an explicit polished PWA review/confirm step. It revalidates credential, source, and writable-folder readiness, shows safe ordered source metadata, size and duration when available, selected language/diarization, destination names, and process/skip planning without echoing source/folder identifiers or URLs. The preview fails closed on malformed DTOs and is invalidated by edits. Existing-result authority is reported honestly as unavailable until the catalog/duplicate-protection workstream; staged runtime progress remains the next split item. Full PWA tests (`247 passed`), pure preflight tests (`2 passed`), portable Python tests (`688 passed, 6 skipped`), TypeScript, lint, production build, lightweight checks, and diff checks pass; PostgreSQL-backed endpoint tests are present but require service-backed CI.
...
| Studio source breadth | **about 93%** | The current branch adds typed transcription options, validated multi-source intake, video/long-media preparation, and explicit safe batch preflight to the existing ElevenLabs workflow and lifecycle foundation; staged progress, catalog-backed duplicate decisions, analytics, and rollout evidence remain. |
...
| Studio combined v1 readiness | **about 79% ±5** | Weighted planning estimate across product, parity, quality, operations, and production evidence. |
