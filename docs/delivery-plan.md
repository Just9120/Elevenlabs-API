...
- 👉 `PWA-JOB-PROGRESS-02` — active on `codex/pwa-job-progress-02`. The candidate keeps a newly terminal job visible across refresh/project switches until the owner explicitly selects **Убрать в историю**, preserves its last confirmed progress while other jobs continue polling, calculates percentage only from confirmed checkpoints, and persists safe terminal-dismissal plus completed/total ElevenLabs part state through additive migration `0018_job_part_progress`. Local source gates are complete; CI, merge, protected migration, API/worker deployment, and production canary are absent.
...
| `PWA-JOB-PROGRESS-02` local source candidate | **100% (`4/4`)** | Durable terminal visibility/dismissal, concurrent checkpoint continuity, durable N/M parts, and focused validation/documentation are present on the branch. Service-backed endpoint/migration execution remains CI evidence, not a local pass. |
...
- Branch: `codex/pwa-job-progress-02`, based on clean `main@bd8d513`; it is `0 behind / 15 ahead` before the final corrective commit.
