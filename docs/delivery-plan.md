...
- ⛔ `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Blocked before approved-folder selection. Verified snapshot `15ca68cc3e4f` protected the separately authorized migration from `0015_user_source_retention` to `0016_transcript_catalog_entries`; the post-check found the new catalog table with zero rows. API-only deployment run `30263481907` verified revision equality, intended image identity, and health at `main@93de8cf`. Authenticated Picker attempts then failed before selection, with Google Chrome reporting an invalid API key. After the operator corrected the Picker key layout in the runtime `.env`, API-only run `30287692026` force-recreated only `studio-api`, reverified dependency health, database/head equality, running image identity, and localhost/public health at the same exact main revision. An authenticated Chrome Picker retry remains required before dry-run; one bounded apply still requires separate explicit authorization.
...
1. Retry production Google Picker in authenticated Chrome after the API-only runtime-config refresh, then run an approved-folder catalog dry-run and require separate authorization for one bounded apply.
...
- Catalog production rollout is paused after verified database/API rollout and before Google discovery. The controlled browser could not complete Picker loading, and a repeated authenticated attempt in the operator's Google Chrome reached Google Picker but reported an invalid API key. No folder or document was selected or read, and neither dry-run nor apply started. The operator corrected the runtime `.env` key layout and run `30287692026` recreated the configuration-consuming API with green identity/health checks; authenticated Chrome must now prove whether Picker accepts the refreshed runtime. Do not infer authorization for apply from the backup, migration, deployment, configuration repair, Picker opening, or dry-run.
