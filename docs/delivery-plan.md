...
- 👉 `PWA-GATED-MIGRATION-CD-01 / VPS bootstrap portability fix` — PR #197 merged the disabled-by-default protected lane as `7c78b705`; exact-main repository and Studio CI passed. Environment `studio-production-migration` now exists and contains the three dedicated secret names, while `STUDIO_MIGRATION_RELEASE_ENABLED` remains absent. The first read-only VPS preflight stopped before mutation because the host lacks PostgreSQL 17 `pg_restore` and Ubuntu exposes only client 16. This branch removes that host-package dependency by validating the restored dump in a network-disabled, read-only helper container from the exact running PostgreSQL image ID. The VPS wrapper/key, enabled lane, migration, and API rollout remain absent.
...
- The current local branch changes only the guarded dump-validation runtime boundary and its documentation/tests. It does not alter the product contract, configure production, install the VPS wrapper/key, enable the lane, apply a migration, deploy API, grant maintenance OAuth, or run either maintenance operation.
...
| Selected Studio v1 source/CI | **100% merged (`40/40`)** | The current maintenance contract and product implementation remain complete; the guarded CD infrastructure is merged at `7c78b705`. This portability fix adds no product capability and cannot increase product readiness. |
...
1. Finish the narrow `PWA-GATED-MIGRATION-CD-01` dump-validation portability fix, pass exact-head CI, merge it, and only then resume the separately approved one-time VPS wrapper/key setup; do not count setup source as production migration evidence.
