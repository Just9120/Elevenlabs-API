...
- 👉 **PWA-PIPELINE-01-PREP — Internal single-job processing orchestration contract** — active docs-only item; defines the internal synchronous composition contract for one already-leased job without implementing orchestration, workers, queues, public APIs, automatic retry, runtime/deploy changes, or production processing.
- 📋 **PWA-PIPELINE-01A — Internal synchronous single-job orchestrator** — next implementation item only; implement the internal server-only synchronous orchestrator that composes the existing boundaries for one already-leased job according to the PWA-PIPELINE-01-PREP contract.
...
The active recommended item is **PWA-PIPELINE-01-PREP — Internal single-job processing orchestration contract**. It is docs-only and defines how a later internal synchronous orchestrator may compose existing job claim/lifecycle, source materialization, provider, Google Docs creation, output persistence, and fenced completion boundaries for one already-leased job. It must not implement the orchestrator, add a worker, public processing API, output browser API, manifest behavior, automatic retry/reconciliation, runtime/deploy changes, production migration execution, or production-live processing claim.

The only explicitly named next implementation item is **PWA-PIPELINE-01A — Internal synchronous single-job orchestrator**. No worker/runtime follow-up is newly approved by this update.
