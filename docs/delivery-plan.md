...
- 👉 `CI-ACTIONS-RUNTIME-01` — Replace every repository-owned use of the deprecated Node.js 20 GitHub Action majors with current Node.js 24 majors while preserving workflow triggers, permissions, commands, cache inputs, artifacts, and deployment boundaries. Local commit `6e3cd67` and its runtime contract are complete; PR CI remains the remote acceptance gate.
...
- PR #187 local validation included full ESLint, TypeScript, production Vite/PWA build, Playwright discovery, focused contracts, and portable Python. The current CI-maintenance commit additionally passes YAML parsing for all seven workflows, `14 passed, 2 skipped` focused workflow contracts, `716 passed, 6 skipped` portable Python, lightweight repository checks, and `git diff --check`. The Windows host has no usable `bash`, so GitHub CI remains authoritative for Linux shell/deploy integration and execution of the upgraded actions.
