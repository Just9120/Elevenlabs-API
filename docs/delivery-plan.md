...
- 🚨 `PWA-PROJECT-CREATE-NAVIGATION-RACE-01` — Active item. Studio run `30202031078` failed its first authenticated Chromium scenario because the project form did not remain open after the explicit `Новый проект` click. Exact-main run `30207923262` later passed all nine scenarios at documentation-only revision `c02accd`; because application code did not change, the pass is evidence of intermittency, not a fix. Make explicit create supersede pending `browse`, add a delayed-projects regression test, and prove the exact changed revision.
...
The scoring method and per-epic fractions are in `docs/audits/repository-audit-2026-07-26.md`. Documentation reconciliation and a same-code green browser rerun do not raise these estimates. Catalog production readiness changes only after the separately evidenced backup, migration, API deployment, dry-run, and authorized apply gates.
