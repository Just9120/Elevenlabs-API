...
- 👉 **PWA-OUTPUT-02-PREP — Browser-safe Studio job output discovery contract** — active docs-only item; defines the authenticated, owner-scoped, read-only browser API contract for discovering persisted Studio job outputs without implementing the endpoint, frontend, runtime behavior, deployment, or production processing.
- 📋 **PWA-OUTPUT-02A — Browser-safe Studio job output API** — only approved next implementation item after this PREP contract.
...
The active recommended item is **PWA-OUTPUT-02-PREP — Browser-safe Studio job output discovery contract**. This documentation-only item defines `GET /api/jobs/{job_id}/outputs` as the future authenticated, owner-scoped, read-only browser API for persisted output discovery, with safe fields, forbidden fields, URL validation, partial-output semantics, deterministic ordering, error behavior, tests, non-goals, and implementation boundaries. It must not implement API routes, serializers, tests, frontend behavior, processing behavior, worker changes, Compose changes, deployment, secrets, or production rollout.
