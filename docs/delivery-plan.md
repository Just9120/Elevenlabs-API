...
- 👉 `PWA-TRANSCRIPT-CATALOG-MIGRATION-01` — The 15-commit source candidate is ready for PR: browser-safe dry-run, separately confirmed idempotent apply, eligible in-place `transcript_doc_v1.2` standardization, minimal durable Studio catalog metadata, linked-catalog duplicate authority, explicit conflict outcomes, and a fail-closed PWA entry point are implemented. The next gates are PR CI, manual migration/deployment, an authenticated live dry-run canary, and a separately authorized apply.
