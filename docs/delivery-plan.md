...
- 👉 **PWA-BATCH-COMPOSER-TAB-STATE-01 — Preserve preparation draft across project tabs** — active in-PR item in PR #150. It fixes the production-confirmed regression where switching `Подготовка → Обзор → Подготовка` reset the unsent row-composer draft within the same selected project and page lifetime.
- ⏸️ **PWA-PROCESSING-ROLLOUT-01A — Manual Studio processing rollout and controlled smoke validation** — paused behind PR #150 and a post-deployment browser recheck. Previous operator rollout validation recorded migration-applied/API-health/worker-idle evidence, but worker state/image revalidation has not been performed after the latest web deployment, controlled smoke has not been run, no provider request or Google document creation occurred, and no production-live processing claim is made.
...
## Active in-PR item: PWA-BATCH-COMPOSER-TAB-STATE-01 — Preserve preparation draft across project tabs

Status: in PR #150. The item fixes the production-confirmed regression where switching `Подготовка → Обзор → Подготовка` reset the unsent preparation composer draft even though there was no page reload and no project switch. PR #150 changed-file list: `apps/studio/src/App.tsx`, `apps/studio/src/App.test.tsx`, and `docs/delivery-plan.md`.
...
- Recorded in the previous operator rollout validation: exactly one `studio-worker` was manually started; at that historical validation point the worker image matched the active API image, no HTTP ports were published, the worker remained running with restart count zero, the worker was observed idle, and no queued or processing jobs existed during idle validation. This worker state and image identity have not been revalidated after the latest web deployment, so this evidence must not imply the worker is necessarily still running now or that the worker image still matches the current API image.
- Factual web deployment evidence: the VPS checkout advanced from `d49400ffbdbf7eef139aa07a4eb3f363f022fb15` to `6b112e07b513d77faaa9fe392fc60f7b150353ce`; commit `6b112e07...` contains the PR #148 frontend plus the docs-only PR #149 reconciliation. The standard component script built and force-recreated only `studio-web`; running image identity verification passed; localhost `/healthz` returned `ok`; `STUDIO_PLATFORM_WEB_DEPLOY_OK` was emitted; API, worker, PostgreSQL, Redis, and migrations were untouched.
- Partial browser acceptance after that deployment confirmed exactly two project tabs: `Обзор` and `Подготовка`; the row-first composer is visible; one empty row appears initially; and source and destination-folder controls are together in the row. This is partial browser acceptance only and must not be treated as complete browser acceptance.
- Production blocker discovered after that partial browser acceptance: adding/editing rows and switching to `Обзор`, then back to `Подготовка`, resets the unsent draft.
- Not run: worker state/image revalidation has not been performed after the web deployment, the controlled production processing smoke has not been run, no provider request or Google document creation occurred, and no `production-live` Studio processing claim is allowed yet.

Rollout boundary:

- Complete and merge PR #150 before resuming rollout acceptance.
- Deploy only `studio-web` for PR #150 after merge.
- Require the normal Studio web deploy success boundary: `STUDIO_PLATFORM_WEB_DEPLOY_OK`.
- Repeat the tab-switch draft-preservation browser scenario (`Подготовка → Обзор → Подготовка`) and confirm the unsent composer draft is preserved.
- Complete the remaining browser acceptance for the row-first composer only after the draft-preservation scenario passes.
- Only then resume worker state/image-identity revalidation and the single bounded controlled smoke.
...
**PWA-BATCH-COMPOSER-TAB-STATE-01 — Preserve preparation draft across project tabs** is the active recommended next item and is in PR #150. PWA-PROCESSING-ROLLOUT-01A is paused behind PR #150 and a post-deployment browser recheck. The required sequence is:

1. Complete and merge PR #150.
2. Deploy only `studio-web`.
3. Require `STUDIO_PLATFORM_WEB_DEPLOY_OK`.
4. Repeat the tab-switch draft-preservation browser scenario (`Подготовка → Обзор → Подготовка`).
5. Complete remaining browser acceptance.
6. Only then resume worker revalidation and the single bounded smoke.

Latest recorded operator rollout evidence includes the successful `studio-web`-only deployment of commit `6b112e07b513d77faaa9fe392fc60f7b150353ce` and partial browser acceptance listed above, but the production draft-reset blocker prevents complete browser acceptance. Worker state/image revalidation has not been performed, controlled smoke has not been run, no provider request or Google document creation occurred, and no production-live processing claim is made. This PR #150 follow-up docs update itself does not deploy, dispatch deployment workflows, start or stop workers, inspect production workers, access production hosts, create jobs, call providers, call Google APIs, perform production smoke, create or delete Google documents, perform cleanup, perform rollback, retry/requeue/reset jobs, run another migration, modify secrets, downgrade the database, remove volumes, or perform destructive Docker Compose operations.
