...
- 👉 `PWA-PICKER-REFERRER-POLICY-01` — PR #194 merged the privacy-minimizing public `origin` policy while preserving explicit `no-referrer` on presigned local-upload PUTs. Exact-main CI passed at `202deed`; the VPS checkout was fast-forwarded to that revision, the active Studio security-header snippet was backed up, `nginx -t` and reload passed, local/public TLS returned exactly `Referrer-Policy: origin`, and both public health endpoints remained HTTP 200. Only authenticated Chrome Picker proof remains.
- ⛔ `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Blocked before approved-folder selection pending the authenticated Chrome Picker retry. Verified snapshot `15ca68cc3e4f` protected the separately authorized migration from `0015_user_source_retention` to `0016_transcript_catalog_entries`; the post-check found the new catalog table with zero rows. API-only deployment runs `30263481907` and `30287692026` verified revision equality, intended image identity, configuration-consuming API recreation, and health at `main@93de8cf`. The merged edge-policy repair is now live and verified; authenticated dry-run and one separately authorized bounded apply still remain.
...
- Repository head, production PostgreSQL, and the deployed API agree on `0016_transcript_catalog_entries`. The merged public edge repair is also live: local and public TLS now expose only the production origin, without path/query data, while public web/API health remains green. Catalog production proof still requires authenticated Picker and approved-folder dry-run evidence plus a separately authorized apply.
...
The scoring method and partial-gate rule are in `docs/audits/repository-audit-2026-07-26.md`. The authoritative 99% source/CI estimate is unchanged and is now backed by exact-main `202deed` checks. Catalog rollout remains `3/5`: the verified edge repair restores the intended prerequisite but does not substitute for authenticated Picker, dry-run, or apply evidence. Studio production evidence therefore remains 61%.

## Product roadmap after production proof

Order work so each capability inherits a known source and production baseline:

1. Retry production Google Picker in authenticated Chrome, then run an approved-folder catalog dry-run and require separate authorization for one bounded apply.
...
- Catalog production rollout is paused after verified database/API/edge rollout and before Google discovery. No folder or document was selected or read, and neither dry-run nor apply started. Authenticated Chrome must now prove Picker open/select/cancel with the live origin policy. Do not infer authorization for catalog apply from the backup, migration, deployment, configuration repair, edge-policy repair, Picker opening, or dry-run.
- PR #194 automated review identified that `docs/project-spec.md` still describes production `0016` migration and intended API deployment as unproven. That higher-priority factual status drift remains pending explicit user authorization to edit the product contract.
