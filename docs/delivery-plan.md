...
The active recommended item is **PWA-JOBS-02A — Studio job lifecycle guardrails for existing record-only APIs**. It is a small backend code+tests slice that centralizes and verifies existing Studio job record lifecycle semantics after the PWA-JOBS-02-PREP processing contract in `docs/studio-jobs-processing-contract.md`.

PWA-JOBS-02A must preserve the existing API contract and stay record-only. It may add a small internal lifecycle helper and focused backend tests for queued creation, safe list/detail metadata, explicit cancellation behavior, credential identity-only handling, safe failure metadata, and source ownership/eligibility checks.
