...
- 👉 `PWA-PICKER-REFERRER-POLICY-01` — VPS probes proved that the configured key, App ID, OAuth project, container environment, and API settings agree. Google rejects the key only when the HTTP referrer is absent and accepts the configured production origin; the live public `Referrer-Policy: no-referrer` is therefore the confirmed blocker. Repository source now uses the privacy-minimizing `origin` policy and preserves explicit `no-referrer` on presigned local-upload PUTs. CI, merge, explicit host-nginx apply/`nginx -t`/reload, live-header verification, and authenticated Chrome Picker proof remain.
- ⛔ `PWA-TRANSCRIPT-CATALOG-MIGRATION-01 / production rollout` — Blocked before approved-folder selection by the confirmed public referrer-policy mismatch above. Verified snapshot `15ca68cc3e4f` protected the separately authorized migration from `0015_user_source_retention` to `0016_transcript_catalog_entries`; the post-check found the new catalog table with zero rows. API-only deployment runs `30263481907` and `30287692026` verified revision equality, intended image identity, configuration-consuming API recreation, and health at `main@93de8cf`. After the edge-policy fix is merged and applied, an authenticated Chrome Picker retry remains required before dry-run; one bounded apply still requires separate explicit authorization.
...
- Repository head, production PostgreSQL, and the deployed API now agree on `0016_transcript_catalog_entries`. VPS probes isolated the remaining pre-dry-run blocker to the live `no-referrer` edge policy: Google rejected the key without a referrer and accepted the same key with the configured production origin. Catalog production proof still requires the merged/applied edge fix, an authenticated approved-folder dry-run, and a separately authorized apply.
...
1. Merge and explicitly apply the origin-only host-nginx referrer policy, verify the live header, retry production Google Picker in authenticated Chrome, then run an approved-folder catalog dry-run and require separate authorization for one bounded apply.
...
- Catalog production rollout is paused after verified database/API rollout and before Google discovery. No folder or document was selected or read, and neither dry-run nor apply started. Safe VPS probes confirmed one Picker key/App ID assignment, exact environment/settings agreement, matching OAuth project, and successful Google key validation when the configured production origin is supplied. The same key failed without a referrer as `API_KEY_HTTP_REFERRER_BLOCKED`; the live site returned `Referrer-Policy: no-referrer`. The source fix changes only the public policy to `origin`, retaining explicit `no-referrer` for presigned local-upload PUTs. It still requires CI, merge, separately applied host nginx with `nginx -t`, live-header verification, and authenticated Chrome proof. Do not infer authorization for catalog apply from the backup, migration, deployment, configuration repair, edge-policy repair, Picker opening, or dry-run.
