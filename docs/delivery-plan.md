...
- Studio has broad source-level implementation at merged `main` revision `625cd33`. Gates 4–7 remain complete: the real one-output ElevenLabs-to-Google-Docs path is production-proven, PR #187 established repeatable secretless browser coverage around the principal preparation/job-result safety boundaries, PR #189 deployed the browser-safe provider-attempt preflight authority, and PR #190 merged the one-time catalog migration/standardization source. The latest browser regression is a new stabilization blocker, not evidence that the earlier bounded provider/Google canary failed.
...
- The authenticated Playwright suite uses real Chromium with isolated FastAPI/PostgreSQL/Redis and controlled external boundaries. It does not call ElevenLabs, Google, S3/R2, or production and therefore does not replace the controlled canary. Post-merge run `30202031078` currently fails before the project form opens; until fixed, exact-main browser coverage is red and eight later scenarios have no evidence for this revision.
...
| Studio source breadth | **about 98%** | Selected processing, safety, catalog migration/standardization, analytics, duplicate/provider-attempt authority, and provider/Google paths are implemented. The project-creation navigation race, golden parity fixtures, accepted-output reuse/skip design, and later maintainability work remain. |
| Studio UX readiness | **about 95%** | Picker roles, local upload, prerequisites, cleanup feedback, CSP behavior, one real completed result, progress/analytics, provider uncertainty, and catalog controls exist. Exact-main browser CI is currently red at project creation, and catalog dry-run/apply has no authenticated production evidence. |
| Studio bounded core production evidence | **about 99%** | The `0015` core baseline has backup, component identities, database compatibility, public health/security, clean smoke, one exactly-one-output canary, safe diagnostics/reconciliation, and deployed preflight authority. This estimate does not include the unrolled `0016` catalog revision or broader workload canaries. |
...
| Studio combined selected-v1 readiness | **about 94–95%** | Core processing is bounded-canary proven, but exact-main browser CI must be restored and the catalog production evidence pipeline is incomplete. Dedicated auto-detect, diarization, video, long-media, and multi-file production canaries also remain. |

Documentation reconciliation does not raise these estimates. A green exact-main browser run will restore validation confidence but does not deploy the catalog; catalog production readiness changes only after the separately evidenced backup, migration, API deployment, dry-run, and authorized apply gates.
