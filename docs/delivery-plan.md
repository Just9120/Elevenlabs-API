...
- 👉 **PWA-JOBS-04A — Processing-time credential and output-destination authorization boundary** — active/in PR: adds internal server-only prerequisites for leased `processing` jobs by decrypting the current owner-scoped provider credential ephemerally and verifying the configured Google Drive output folder is still accessible, a folder, and writable. It still adds no provider request, source materialization, worker, queue consumer, Google Docs output, transcript persistence, completed transition, manifest mutation, public endpoint, frontend, migration, runtime/deploy/production change, or secrets change.
...
The active recommended item is **PWA-JOBS-04A — Processing-time credential and output-destination authorization boundary**. It is an internal prerequisites boundary for jobs already in `processing`: exact lease owner/generation and active lease are required; cancellation, missing/archived project state, credential mutation, and output-folder mutation block the ephemeral context. It resolves/decrypts only the current owner-scoped BYOK provider credential, verifies only the project-configured Drive output folder identity/type/writability, revalidates after decryption and Google metadata I/O, and does not authorize provider execution, source materialization, or output creation.

Explicit non-goals: no new documentation files; no worker implementation; no queue consumer; no provider API calls; no source materialization; no Google Docs API calls or output creation; no transcript/output persistence; no manifest mutation implementation; no retry runner/background tasks; no public API endpoint; no new job status values beyond existing `queued`/`processing`/terminal states; no CI/CD workflow changes; no Docker/Compose/runtime/deploy changes; no production deployment or rollout; no automatic migration rollout; no VPS access; no secrets, tokens, environment values, file-mounted secret contents, transcript bodies, Google Docs body content, raw provider payloads, private storage keys, Drive IDs, or presigned URLs.
