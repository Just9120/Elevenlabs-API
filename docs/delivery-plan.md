...
- ✅ **PWA-BATCH-COMPOSER-ROW-UX-01 — Inline source and destination selection inside repeatable composer rows** — source-done/merged through PR #148 (source head `f9ba5203c70185370eb539828b7dcaa911977549`, merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`); Backend CI #359 passed and Studio PWA CI #184 passed. The merged source changes `Подготовка` to a row-first composer where source selection and destination-folder selection are presented together inside repeatable preparation rows, Google Drive and local multi-selection expand into ordered rows, project files are secondary to the composer, and explicit atomic `/jobs/batch` submission is preserved. This is a source/CI statement only; deployment, browser acceptance, worker revalidation, controlled smoke, and production-live processing remain unverified.
- 👉 **PWA-PROCESSING-ROLLOUT-01A — Manual Studio processing rollout and controlled smoke validation** — active `manual-ops-required` item. Previous operator rollout validation recorded migration-applied/API-health/worker-idle evidence, and browser acceptance after PR #146 exposed the split intake/composer UX defect that PR #148 corrected in source. The item now requires PR #148 web-deployment evidence, browser acceptance of the row-first UX, worker state/image-identity revalidation, and then exactly one bounded controlled processing smoke. No production-live processing claim is made.
...
## Last completed item: PWA-BATCH-COMPOSER-ROW-UX-01 — Inline source and destination selection inside repeatable composer rows

Status: source-done/merged through PR #148. Source head `f9ba5203c70185370eb539828b7dcaa911977549` merged as `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`; Backend CI #359 and Studio PWA CI #184 passed. Source and CI are complete for the row-first composer correction, while deployment, browser acceptance, worker revalidation, controlled smoke, and runtime validation remain separate and unverified.
...
- Recorded in the previous operator rollout validation: exactly one `studio-worker` was manually started; at that historical validation point the worker image matched the active API image, no HTTP ports were published, the worker remained running with restart count zero, the worker was observed idle, and no queued or processing jobs existed during idle validation. This worker state and image identity have not been revalidated after PR #148 merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`, so this evidence must not imply the worker is necessarily still running now or that the worker image still matches the current API image.
- Confirmed after PR #146 merge commit `16ecbf2a6f23180bacdf412fa94901069806aea0`: browser acceptance showed the selected project exposes exactly `Обзор` and `Подготовка`, and the PR #146 composer frontend was deployed and visible.
- Exposed after PR #146: browser acceptance found the split source-intake/composer UX defect in `Подготовка`; this evidence validates the old PR #146 deployment only. It does not validate that the PR #148 row-first frontend is deployed or visible.
...
- Active rollout now requires factual `studio-web` deployment evidence for PR #148 merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`.
- If automatic Studio Platform CD did not deploy that frontend commit, manually dispatch only the `web` component.
- Require the normal Studio web deploy success boundary: `STUDIO_PLATFORM_WEB_DEPLOY_OK`.
- Browser acceptance of the merged row-first UX must confirm exactly two project tabs: `Обзор` and `Подготовка`; source and destination-folder selection presented together inside each preparation row; one empty preparation row initially; Google Drive and local multi-selection expanding into additional ordered rows; an independent destination-folder selector on every row; existing project sources selectable at row level; `Файлы проекта` secondary to the composer; `Текущие и недавние задачи` below the composer; and no old standalone source-intake-first workflow remains. These checks have not passed yet.
- Worker startup has historical operator evidence as `worker-running`, but this is not a production-live processing claim, and worker state/image identity must be revalidated after PR #148.
- Controlled production processing smoke remains separate and not run. It must remain exactly one bounded smoke using one operator-approved account/project, one small supported source, one active owner-scoped ElevenLabs credential, one authenticated Google connection, one writable selected output folder, one composer row, one resulting job, and no automatic retry.
- Provider requests remain separate and have not been run by this reconciliation.
- Google document creation remains separate and has not been run by this reconciliation.
- Exactly-once output validation remains separate and has not been run by this reconciliation.
- Cleanup and rollback remain separate and have not been run by this reconciliation.
- Job retry, requeue, and reset remain separate and have not been run by this reconciliation.
- Manifest parity remains separate and has not been run by this reconciliation.
...
Studio PWA is the current development contour intended to duplicate Google Colab product scope with PWA/platform adaptations. Source on main now includes account/session/BYOK, projects, sources, Google Drive OAuth/metadata/folder-child selection, local temporary upload intake, persisted job records, job UI, preflight/claim-readiness guardrails, internal processing-time source materialization, processing prerequisites, and the PR #148 row-first `Подготовка` composer with atomic `/jobs/batch` submission. Browser evidence after PR #146 confirms only that the old PR #146 deployed project workspace was visible with exactly `Обзор` and `Подготовка`, and that it exposed the split intake/composer UX defect later corrected by PR #148 source. PR #148 is source-done/merged and CI-verified, but its frontend deployment, browser visibility, browser acceptance, worker state/image identity, controlled smoke, and production-live processing remain unverified. PWA-WORKER-01A is source-done/merged via PR #124 (head `6e858a066304b3bf20388322c81c293f61df49ff`, merge `99f0e34e67740a60053242e98e194f888ff05341`) with Backend CI #293 passed and Studio PWA CI #125 passed. PWA-WORKER-01-PREP is source-done/merged via PR #125 (head `a5694a13db1243b12bd93705b280057cbc35bbfc`, merge `e850f6f6cd68dde8ebb55c8008bcc19f1c9750c4`) with Backend CI #296 passed. PWA-WORKER-01B is source-done/merged via PR #126 (head `b0236d1f6eab4495d2b1d315f4dc06f6c38055a5`, merge `4b91598651c90199ffb189b3758f9bfab6a05d11`) with Backend CI #299 passed and Studio PWA CI #128 passed. Current Studio source includes a worker entrypoint and Compose service definition, plus frontend output rendering source for an explicitly opened job, but this does not mean production deployment, production-live processing, manifest mutation, or exactly-once Google document creation is claimed. PWA-OUTPUT-02-PREP is source-done/merged via PR #127 (head `709ee29b2323c91a34683ce4d2b70804c7a6c4bc`, merge `fc856758da6e08d2fbdb42ece5004d6a9117cd15`) with Backend CI #301 passed. PWA-OUTPUT-02A is source-done/merged via PR #128 (head `3b713f2f0defee1a9ff08c5177f29277637e5b2b`, merge `80e29cab9db33a497fe2d3c71d31c34f7bd8de0c`) with Backend CI #304 passed and Studio PWA CI #131 passed. PWA-OUTPUT-02B is source-done/merged via PR #129 (head `a3ea0117b525970cdeec8235c3d740b1160387cb`, merge `7d555faa3e6e02d41327d6761f6bda1c2c7d59dd`) with Backend CI #307 passed and Studio PWA CI #134 passed. PWA-PROCESSING-ROLLOUT-01-PREP is source-done/merged via PR #130 (head `ba87fe7a991b748b8f1fbc0eed3e5b2d295622c0`, merge `41dfa926f77b2a8657dec1d675d770000b188843`) with Backend CI #310 passed and Studio PWA CI #137 passed. PWA-PROCESSING-ROLLOUT-01A is now active/manual-ops-required. It has partial historical operator rollout evidence: production database migration `0009_job_output_destinations` was applied, API health reported database reachable and migrations current, and exactly one worker was observed running and idle at the previous validation point. Exactly one worker was historically observed as worker-running/idle with restart count zero and no published HTTP ports, but worker state and image identity have not been revalidated after PR #148 merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`. Controlled production smoke remains not run; no production-live Studio processing is claimed.
...
**PWA-PROCESSING-ROLLOUT-01A — Manual Studio processing rollout and controlled smoke validation** is the active recommended next item with status `manual-ops-required`. The required sequence is:

1. Establish factual web-deployment evidence for PR #148 merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`.
...
4. Perform browser acceptance of the merged row-first composer, confirming exactly two project tabs (`Обзор`, `Подготовка`); source and destination-folder selection together inside each preparation row; one initially empty preparation row; Google Drive and local multi-selection expanding into additional ordered rows; independent destination-folder selectors per row; existing project sources selectable at row level; `Файлы проекта` secondary to the composer; `Текущие и недавние задачи` below the composer; and no old standalone source-intake-first workflow.
5. Revalidate worker state and worker image identity after PR #148.
6. Only then run exactly one bounded controlled smoke using one operator-approved account/project, one small supported source, one active owner-scoped ElevenLabs credential, one authenticated Google connection, one writable selected output folder, one composer row, one resulting job, and no automatic retry.

Latest recorded operator rollout evidence is limited to migration-applied for production database revision `0009_job_output_destinations`, API health reporting database reachable/migrations current, and exactly one manually started `studio-worker` observed running/idle with no published HTTP ports, restart count zero, and image identity matching the active API image at the historical validation time. The worker state and image identity were observed only at that validation time and have not been revalidated after PR #148 merge commit `d49400ffbdbf7eef139aa07a4eb3f363f022fb15`; do not infer the worker is necessarily still running now. PR #148 web deployment, browser acceptance, worker revalidation, and controlled processing smoke remain unverified, and controlled smoke remains not-run. This docs PR itself does not deploy, dispatch deployment workflows, start or stop workers, inspect production workers, access production hosts, create jobs, call providers, call Google APIs, perform production smoke, create or delete Google documents, perform cleanup, perform rollback, retry/requeue/reset jobs, run another migration, modify secrets, downgrade the database, remove volumes, or perform destructive Docker Compose operations.
