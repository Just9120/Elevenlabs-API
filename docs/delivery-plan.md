...
Continue local PWA stability hardening with a focused audit of the account retention preference boundary. Verify runtime response validation, 15-second read and 20-second PATCH deadlines, synchronous duplicate prevention, safe treatment of ambiguous updates through one authoritative preferences read, last-known-state preservation, and non-raw error rendering. Keep Google OAuth/connection outside this item; do not change retention business rules, backend API/schema, storage cleanup, deployment, or production state without separate evidence.
