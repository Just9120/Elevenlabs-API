...
- 👉 **PWA-JOBS-02-PREP — Studio job processing execution contract** — current recommended docs/design-only preparation item: document the future worker/provider execution boundary before any coding slice implements processing. This item is coding-only and repository-scoped while production migration rollout remains manual/operator-scoped.
...
The active recommended item is **PWA-JOBS-02-PREP — Studio job processing execution contract**. It is a docs/design-only preparation item that defines the high-level contract for a future worker/provider execution slice without implementing runtime processing. The focused design document is `docs/studio-jobs-processing-contract.md`.

PWA-JOBS-02-PREP must stay coding-only and safe while production migration rollout remains manual/operator-scoped. It may clarify repository documentation, but it must not touch application runtime behavior, CI/CD, Docker/Compose, deploy scripts, stateful services, migrations, secrets, or production infrastructure.

Acceptance criteria:

- future Studio job lifecycle/state transition contract is documented at a high level;
- credential boundary is documented: the browser never receives raw credentials, and a future worker may decrypt only immediately before a provider request;
- source access boundary is documented for existing source records only;
- output boundary is documented: Google Docs output creation, output persistence, and manifest mutation are future separate slices;
- failure/safe metadata boundary is documented without raw provider payloads, transcript body, Google Docs body content, tokens, secrets, or file-mounted secret contents;
- rollout blocker is documented: production migration/manual operations remain separate and are not claimed by this PR.

Explicit non-goals: no worker implementation; no queue consumer; no provider API calls; no credential decryption/use at runtime; no Google Docs output creation; no output persistence; no manifest mutation; no backend schema changes or Alembic migrations; no CI/CD changes; no Docker/Compose/runtime changes; no production deployment; no automatic migration rollout; no secrets, tokens, environment values, or file-mounted secret contents.
