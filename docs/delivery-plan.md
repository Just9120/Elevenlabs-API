...
- `main` and `origin/main` were synchronized at `0b23320` after PR #188. The active `codex/pwa-preflight-uncertainty` branch starts from that exact revision and is ten focused commits ahead at the local source-complete checkpoint.
...
- The active preflight-authority batch passes the full locally applicable gate: `727 passed, 6 skipped` portable Python, all 279 Studio Vitest tests, ESLint, TypeScript, production Vite/PWA build, Playwright discovery with eight authenticated scenarios, lightweight repository checks, and `git diff --check`. Focused backend authority/worker-guard coverage passes locally (`64 passed`). The new PostgreSQL API scenarios compile and collect, but their execution plus the existing authenticated Chromium suite remain GitHub CI acceptance evidence.
