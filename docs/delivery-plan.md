...
**PWA-BATCH-COMPOSER-01 — Combine preparation UX and create source-to-folder rows** is currently in PR #146 and is not merged. It keeps the normal platform project workspace as `Обзор` plus combined `Подготовка`, where source intake, ordered source→output-folder composer rows, and current/recent job history are visible together and normal creation uses the atomic `/jobs/batch` endpoint. Operator runtime smoke remains the next recommended item only after review and merge; this PR does not start workers, deploy, call providers, create Google Docs, run production smoke, perform cleanup, perform rollback, or claim manifest parity.
