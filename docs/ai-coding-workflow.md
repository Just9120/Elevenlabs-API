...
Документ также фиксирует базовый старт нового репозитория: первичный skeleton документации, README-навигацию, выбор режима workflow / utility / CI/CD, ранние архитектурные решения по данным, БД, Docker и правила периодической проверки масштабирования.


## Quick start для LLM

Этот документ не заменяет проектный контекст. Он объясняет, как с ним работать.

Чтобы не потерять контекст проекта, ChatGPT / reasoning LLM должна сначала определить и прочитать:

1. `/docs/project-spec.md` — что строим и какие требования актуальны.
2. `/docs/delivery-plan.md` — где проект сейчас и какой следующий item.
3. `/docs/ai-coding-workflow.md` — как работать через ChatGPT / Codex / PR / docs.
4. `/docs/ci-cd-rules.md`, если задача затрагивает CI/CD, deploy, VPS, secrets, `.env`, Docker Compose, post-check или rollback.
5. `/docs/context-bundle-builder/MASTER_SPEC.md`, если используется Context Bundle Builder.

Базовая логика:

```text
Workflow задаёт процесс.
Project spec задаёт смысл проекта.
Delivery plan задаёт текущую позицию.
CI/CD rules задают operational boundaries.
Context Bundle Builder, если есть, помогает собрать контекст, но не заменяет source-of-truth документы.
```

Если какого-то документа нет, модель должна явно указать ограничение и не придумывать его содержание.

Если доступен только этот workflow-файл, модель должна воспринимать его как процессную инструкцию, а не как описание конкретного проекта. Для проектных требований всё равно нужен `/docs/project-spec.md` или другой явно предоставленный пользователем source-of-truth.
...
/docs/ci-cd-rules.md, если есть
    Универсальные правила CI/CD, GitHub Actions, deploy, VPS, secrets, post-check и rollback.
```
...
### 3.1. Нулевой старт нового репозитория

До обычного цикла `project-spec.md → delivery-plan.md → Codex prompt → PR → review → merge` применяется нулевой стартовый этап.

Базовая последовательность:

```text
создать GitHub repository
→ дать ChatGPT / reasoning LLM файл AI Coding Workflow
→ подготовить focused prompt для Codex на skeleton документации
→ вручную добавить или синхронизировать актуальное содержание документов
→ определить базовые архитектурные решения
→ дальше работать по обычному workflow
```

Цель нулевого старта — чтобы новая модель, новый участник проекта или coding-agent сразу понимали, где находится source of truth, как устроен процесс и какие решения уже приняты.

#### Skeleton документации

На старте Codex может создать только структуру и placeholders.

Codex не должен придумывать содержание проектного ТЗ, workflow, CI/CD rules, спецификации Context Bundle Builder, архитектурные решения, бизнес-логику или ограничения вместо пользователя.
...
/docs/delivery-plan.md
/docs/ai-coding-workflow.md
/docs/ci-cd-rules.md, если планируется CI/CD, VPS deploy, Docker Compose deploy или production operations
/docs/context-bundle-builder/MASTER_SPEC.md, если пользователь планирует использовать Context Bundle Builder
README.md
```

Правила:

- пустой placeholder не считается полноценным source of truth;
- `project-spec.md` должен быть заполнен пользователем или синхронизирован из актуального ТЗ;
- `delivery-plan.md` должен показывать текущую позицию, даже если на старте в нём только первый bootstrap/checkpoint;
- `ai-coding-workflow.md` должен быть добавлен как рабочие правила репозитория;
- `ci-cd-rules.md` добавляется, если проект использует или планирует использовать CI/CD;
- `MASTER_SPEC.md` для Context Bundle Builder добавляется только если утилита действительно нужна;
- если документ отсутствует, модель должна явно указать ограничение, а не придумывать его содержание.

#### README как входная навигация

`README.md` должен содержать краткое описание проекта и ссылки на ключевые документы с пояснением роли каждого файла.

Минимальная навигационная логика:

```text
Project Specification — что строим и какие требования актуальны.
Delivery Plan — что делаем сейчас и какой следующий item.
AI Coding Workflow — как работаем через ChatGPT / Codex / PR / docs.
CI/CD Rules — как устроены CI, CD, VPS, secrets, deploy, post-check и rollback, если применимо.
Context Bundle Builder — как собирать контекст для LLM, если утилита используется.
```

#### Режим workflow и Context Bundle Builder

Workflow является базовым процессным слоем.

Context Bundle Builder является опциональной repo-local утилитой для сбора контекста. Он не является обязательной зависимостью workflow.
...
- если Context Bundle Builder отсутствует, AI Coding Workflow работает автономно через repository docs, PR diff, tests и CI;
- если Context Bundle Builder присутствует, он помогает собирать контекст, но не заменяет source-of-truth документы;
- generated bundles / archives являются context snapshots, а не source of truth;
- утилиту не нужно запускать как обязательный gate для каждого обычного focused PR;
- утилиту стоит использовать для broad audit, handoff, docs-vs-code audit, component review или когда ручной сбор контекста рискованно неполный.

#### CI/CD baseline при старте

CI обычно подключается сразу при заведении или первичной настройке проекта.

CD подключается позже — когда проект готов к первому deploy или smoke test на VPS.

Если проект будет жить на VPS или аналогичном сервере, первый server bootstrap может использовать repository Deploy Key для доступа VPS к GitHub repository. После первичной настройки дальнейший CD должен выполняться по `/docs/ci-cd-rules.md`.
...
- CD не должен выполнять cleanup, hardening, переносы директорий, destructive-действия или изменения за пределами целевого сервиса;
- если для CD не хватает `APP_DIR`, `EXPECTED_REMOTE`, `EXPECTED_BRANCH`, `COMPOSE_SERVICE`, health endpoint или `DEPLOY_*` secrets, модель должна дать команды для безопасной проверки, а не придумывать значения.

#### Docker baseline

Для серверных проектов нужно сразу проектировать проект с учётом Docker-упаковки.

Если проект живёт на VPS или аналогичном сервере, целевая модель deploy должна предполагать пересборку application container или нескольких containers, если проект разделён на сервисы.

Kubernetes не является базовым требованием и остаётся за рамками старта, если проект явно не требует такого масштаба.

Для проектов, которые исполняются в средах вроде Google Colab, notebook/runtime-only environment или другой внешней managed-среде, Docker и VPS deploy не являются обязательными.

### 3.2. Базовое решение по данным, БД и масштабированию

На старте проекта ChatGPT / reasoning LLM должен определить, требуется ли проекту постоянное хранение данных.

Если БД не требуется, это решение должно быть явно зафиксировано, чтобы Codex не добавлял persistence layer без отдельного требования.

Если БД требуется, нужно выбрать минимально достаточную модель хранения данных и одновременно зафиксировать возможный путь масштабирования.

Главный принцип:

```text
Выбираем минимально достаточную модель хранения данных,
но фиксируем путь масштабирования, чтобы MVP-решение не заблокировало рост проекта.
```

#### Категории данных

Нужно определить, какие данные есть в проекте:
...
Для каждой категории нужно понять:

- где находится canonical state;
- кто может менять данные;
- нужна ли история изменений;
- нужна ли транзакционность;
- есть ли PII / secrets / sensitive data;
- какие данные можно пересоздать;
- какие данные нельзя потерять.

#### System of Record

Для каждого важного типа данных должен быть определён System of Record.

System of Record — это место, где хранится каноническое состояние данных.
...
users → PostgreSQL
orders → PostgreSQL
product_cards → document DB или PostgreSQL JSONB
embeddings → vector DB / pgvector as rebuildable index
cache → Redis
uploaded_files → filesystem / object storage
external_market_data → external API + local cache
```

Правила:

- реляционная БД часто является System of Record для пользователей, транзакций, настроек, заказов и строгих связей, но это нужно фиксировать явно;
- vector DB обычно не является System of Record;
- vector DB должна рассматриваться как semantic index / retrieval layer;
- embeddings должны ссылаться на canonical IDs из основной БД или другого System of Record;
- если vector index можно пересобрать, это нужно явно указать;
- если vector index нельзя безопасно пересобрать, это уже stateful data и требует backup / restore policy.

#### Контуры хранения

Проект может использовать один или несколько контуров хранения:

| Контур | Когда нужен | Примеры |
|---|---|---|
| Реляционный | транзакции, пользователи, заказы, настройки, строгие связи | SQLite, PostgreSQL, MySQL |
| Векторный | RAG, semantic search, embeddings | pgvector, ChromaDB, Qdrant |
| Документный | гибкие документы, карточки, JSON-like структуры | MongoDB, PostgreSQL JSONB |
| Cache / queue | временное состояние, фоновые задачи, rate limits | Redis, RabbitMQ, Celery broker |
| File/object storage | файлы, exports, media, archives | local filesystem, S3-compatible storage |

Polyglot persistence допускается, но не должна добавляться автоматически. Каждый дополнительный контур должен иметь понятную задачу, владельца данных, backup/restore ожидания и влияние на deploy.

#### Стартовый вариант и путь эволюции

Для проекта нужно зафиксировать текущий уровень и возможный следующий уровень.

Примеры:

```text
No DB → SQLite → PostgreSQL
SQLite + local vector store → PostgreSQL + pgvector
PostgreSQL → PostgreSQL + pgvector
PostgreSQL + separate vector DB → managed vector DB
local files → object storage
single app container → app service + stateful services
```
...
Путь эволюции не означает, что следующий уровень нужно внедрять сразу. Он фиксирует направление, чтобы Codex, ChatGPT и будущие PR не принимали несовместимые технические решения.

#### Где фиксировать решение

В `project-spec.md` нужно обновить разделы:

```text
Data and state model
Architecture constraints
Integrations, если есть внешние хранилища или API
Security and safety constraints, если есть sensitive data
Testing and validation requirements
```

Минимально нужно зафиксировать:

- какие БД используются сейчас;
- какие данные где хранятся;
- какой контур является System of Record;
- какие индексы, кэши или vector stores являются производными;
- какие данные можно пересобрать;
- какие данные нельзя потерять;
- какие ограничения есть у выбранной БД;
- какой путь миграции или масштабирования ожидается при росте проекта.

В `delivery-plan.md` эволюция БД должна быть разбита на delivery items.

Пример:

```text
Checkpoint 1 — Базовая persistence model
- внедрить SQLite;
- описать schema;
- добавить минимальные проверки.

Checkpoint 2 — RAG / vector index
- добавить embeddings;
- связать vector records с canonical IDs;
- описать reindex strategy.

Checkpoint 3 — Production DB migration
- перейти на PostgreSQL;
- подготовить миграции;
- обновить deploy configuration;
- проверить backup / restore.
```

Любой переход на новую БД, добавление vector DB, изменение persistence model, миграций, volumes или backup strategy должен обновлять `delivery-plan.md` в том же PR.

#### Периодическая проверка масштабирования БД

Решение по БД принимается на старте проекта, но не считается окончательным навсегда.

По ходу проекта ChatGPT / reasoning LLM должен периодически проверять, не появились ли признаки того, что текущую модель хранения данных нужно масштабировать, заменить или разделить на дополнительные контуры.

Проверка нужна при:

- росте количества пользователей, данных, транзакций или фоновых задач;
- появлении параллельных записей;
- появлении RAG, embeddings, semantic search или vector index;
- замедлении запросов;
- усложнении бизнес-логики;
- появлении отчётности, аналитики или audit trail;
- переходе от MVP к production deploy;
- внедрении CD на VPS;
- появлении backup / restore требований;
- изменении требований к безопасности, PII или sensitive data.

Если текущая БД становится ограничением, модель должна не менять архитектуру сама, а зафиксировать это как architectural risk / scaling candidate и предложить отдельный delivery item.

Примеры:

```text
SQLite → PostgreSQL
local vector store → pgvector / Qdrant / managed vector DB
local files → object storage
single DB → read model / cache / queue
single container → app service + stateful services
```

Переход на новую БД, добавление vector DB, миграции, reindex, backup/restore, перенос volumes или изменение stateful services должны выполняться только как отдельная delivery / maintenance задача с явным scope, rollback и validation.

#### Stateful services protection

Если проект использует VPS, Docker Compose или CD, базы данных и связанные stateful services должны быть защищены.

Стандартный CD не должен:

- пересоздавать базы данных;
- пересоздавать vector DB;
- удалять volumes;
- выполнять `docker compose down`;
- выполнять `docker volume prune`;
- выполнять `rm -rf`;
- менять production `.env`;
- выполнять destructive migrations без отдельной задачи;
- трогать dependency/stateful services, если deploy касается только application service.

CD должен деплоить только целевой application service, если иное явно не оформлено отдельной maintenance / migration задачей.

Миграции БД, backup, restore, перенос volumes, reindex vector DB и cleanup stateful services должны быть отдельными delivery items с явным scope, рисками, rollback-планом и validation.
...
```text
/docs/ai-coding-workflow.md
/docs/ci-cd-rules.md, если задача затрагивает CI/CD, GitHub Actions, deploy, VPS, Docker Compose, secrets, `.env`, post-check или rollback
```
...
### 4.5. CI/CD rules source of truth

Если в репозитории есть отдельный CI/CD rules document, его стандартное расположение:

```text
/docs/ci-cd-rules.md
```

Этот файл является source of truth для:

- границ CI;
- границ CD;
- правил GitHub Actions;
- production/VPS deploy constraints;
- работы с secrets и `.env`;
- Docker Compose deploy rules;
- post-check и rollback expectations.
...
- `/docs/delivery-plan.md` как operational source of truth;
- `/docs/ai-coding-workflow.md` как workflow source of truth;
- `/docs/context-bundle-builder/MASTER_SPEC.md` как devtool source of truth, если утилита используется.

Изменения в `/docs/ci-cd-rules.md` сами по себе не требуют отдельного delivery-документа. Если изменение CI/CD rules становится отдельным рабочим item, оно может быть отражено в `/docs/delivery-plan.md` или `/docs/ai-delivery-infrastructure-plan.md` в зависимости от scope.

### 4.6. Source priority model для LLM и Codex
...
| P1 operations | `/docs/ci-cd-rules.md`, если есть | Правила CI/CD, GitHub Actions, VPS deploy, secrets, post-check и rollback. |
...
- `ci-cd-rules.md` задаёт CI/CD boundaries и не меняет product scope.
...
Skeleton-документы могут быть созданы Codex на нулевом старте, но placeholders не считаются заполненным source of truth. Пользователь или reasoning LLM должны синхронизировать в них актуальное содержание до передачи broad-задач coding-agent.
...
`/docs/ci-cd-rules.md` рекомендуется добавлять, если проект использует или планирует использовать GitHub Actions CI/CD, VPS deploy, Docker Compose deploy или production operations. Этот файл не требует отдельного delivery plan по умолчанию.
...
- [CI/CD Rules](docs/ci-cd-rules.md) — правила CI, CD, GitHub Actions, deploy, VPS, secrets, post-check и rollback.
...
- [Context Bundle Builder](docs/context-bundle-builder/MASTER_SPEC.md) — source of truth для repo-local утилиты сбора контекста, если она используется.
```
...
- связку `/docs/ci-cd-rules.md` с workflow, README и Context Bundle Builder baseline context, если это становится отдельным AI delivery infrastructure item.
...
- Изменения в `/docs/ci-cd-rules.md` не требуют отдельного delivery plan по умолчанию; они трекаются отдельным item только если становятся самостоятельной задачей.
- Конкретное внедрение CI/CD для продукта трекается в `/docs/delivery-plan.md`.
...
- [ ] No CI/CD rules impact
- [ ] `/docs/ci-cd-rules.md` checked because this PR touches CI/CD, GitHub Actions, deploy, VPS, secrets, `.env`, post-check or rollback
- [ ] `/docs/ci-cd-rules.md` updated
...
- [ ] CI/CD rules or deploy workflow assumptions
...
- если PR меняет CI/CD, GitHub Actions, deploy, VPS, Docker Compose deploy, secrets, `.env`, post-check или rollback, PR должен быть проверен против `/docs/ci-cd-rules.md`, если файл есть;
...
### 10.2. Cross-plan update rule

По умолчанию один PR должен относиться к одному delivery-контуру:

- product/project delivery item в `/docs/delivery-plan.md`;
- или AI workflow / Context Bundle Builder infrastructure item в `/docs/ai-delivery-infrastructure-plan.md`, если такой файл используется.

Смешанные PR, которые одновременно меняют product behavior и AI delivery infrastructure, допустимы только как исключение.

Если mixed PR неизбежен, PR body должен явно указать:

- affected product delivery item;
- affected AI delivery infrastructure item;
- какие документы обновлены;
- почему изменения нельзя безопасно разделить на два PR;
- какие validation checks покрывают оба контура.

Codex не должен самовольно объединять product work и AI infrastructure work в один широкий PR ради удобства.
...
- точный формат применения изменения: `diff`-блок или блоки `SEARCH / REPLACE`;
- минимальный контекст вокруг заменяемого фрагмента, достаточный для безопасного применения патча;
- указание, что остальной текст документа не должен пересказываться, сокращаться или регенерироваться.

Codex не должен молча сокращать, обрезать, перегенерировать или заменять большой source-of-truth документ summary-версией.

Proposal-файл должен быть применимым рабочим артефактом, а не эссе или summary. Codex должен предлагать точечную вставку / замену и не пересказывать большой документ своими словами.
...
- перед подготовкой prompt по CI/CD-задачам читать `/docs/ci-cd-rules.md`, если файл есть;
...
- /docs/delivery-plan.md
- /docs/ai-coding-workflow.md
- /docs/ci-cd-rules.md, if the task touches CI/CD, GitHub Actions, deploy, VPS, Docker Compose, secrets, `.env`, post-check or rollback
- README.md
...
- Use P1 operations /docs/ci-cd-rules.md as the source of truth for CI/CD boundaries when the task touches CI/CD.
...
- Keep the PR tied to one delivery contour by default: product/project delivery plan or AI delivery infrastructure plan. Mixed PRs must explicitly justify why both contours are changed together.
- If requesting `component_review` with `target_scope`, use only an exact repository path, a configured component ID, or a known metadata keyword. Do not invent semantic keywords.
...
- If this PR changes CI/CD, GitHub Actions, deploy, VPS, Docker Compose, secrets, `.env`, post-check or rollback, check /docs/ci-cd-rules.md and update it only when the rules themselves change.
...
```text
project-spec.md + delivery-plan.md + ai-coding-workflow.md + ci-cd-rules.md when relevant + diff + tests + CI
```
...
- если PR затрагивает CI/CD, GitHub Actions, deploy или production operations, изменения соответствуют `/docs/ci-cd-rules.md`, если файл есть;
...
- [ ] Если проект использует CI/CD, GitHub Actions deploy, VPS, Docker Compose deploy или production operations, в репозитории есть `/docs/ci-cd-rules.md` или явно принято решение вести эти правила в другом документе.
...
- [ ] `README.md` содержит ссылки на ключевые документы, включая `/docs/ci-cd-rules.md`, если этот файл используется.
...
- [ ] В workflow описан нулевой старт нового репозитория: skeleton документации, ручное наполнение source-of-truth документов, README-навигация и переход к обычному циклу.
- [ ] Новая модель может определить режим репозитория: Workflow only, Builder only или Integrated Workflow + Builder.
- [ ] Новая модель может определить состояние CI/CD: CI отсутствует / CI есть / CD ещё не нужен / CD внедрён / CD требует проверки.
- [ ] На старте проекта фиксируется решение по данным и БД: нужна ли БД, какой System of Record используется, какие индексы/кэши производные и какой путь масштабирования возможен.
- [ ] По ходу проекта модель периодически проверяет, не стала ли текущая БД архитектурным ограничением, и выносит масштабирование в отдельный delivery item.
- [ ] Для серверных проектов описан Docker baseline, а для notebook/runtime-only проектов явно допускается отсутствие Docker/VPS deploy.
- [ ] Stateful services, базы данных, vector DB, queues и volumes защищены от стандартного CD и destructive-действий.
...
- [ ] Workflow объясняет, что по умолчанию один PR относится к одному delivery-контуру, а mixed product + AI infrastructure PR является исключением.
- [ ] Reusable Codex prompt block запрещает выдумывать semantic `target_scope` для Context Bundle Builder и требует path / configured component scope ID / known metadata keyword.
- [ ] Large source-of-truth proposal format требует точный `diff` или `SEARCH / REPLACE`, а не пересказ большого документа.
...
- менять CI/CD, deploy, VPS, secrets или production operations без проверки `/docs/ci-cd-rules.md`, если файл есть;
