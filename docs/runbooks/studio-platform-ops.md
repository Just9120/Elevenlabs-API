...
Stop the worker and avoid automatic retry on database revision mismatch, missing runtime config, unexpected worker startup error, lease expiry or fencing loss, cancellation uncertainty, provider or Google authentication rejection, output side-effect uncertainty, duplicate or unexpected Google document creation, wrong output folder, missing persisted output after external document side effect, unsafe/secret-bearing evidence, or unknown exception/state transition. Any exception between claim commit and the transition to `processing` blocks the smoke and blocks any `production-live` claim.

Compact queued-after-claim troubleshooting scenario: if a job remains `queued`, `claimed_at` and lease fields are set, `attempt_count` is `0`, and the worker logs `studio_worker_iteration_error`, stop the worker, do not create a second task, and do not perform a manual retry. Check readiness, lease timestamps, and timezone awareness. Do not clear the lease with direct SQL. Fix the source, pass CI, deploy the API, recreate the worker only after verifying new image parity, and repeat smoke only with an empty queue and one new task.
...
## Google Picker runtime config

For PWA-GOOGLE-PICKER-01, enable the Google Picker API in the existing Google Cloud project used by Studio OAuth. Configure `STUDIO_GOOGLE_PICKER_API_KEY` with a public browser API key restricted in Google Cloud to the production HTTPS origin/referrer and to the required Google APIs. Configure `STUDIO_GOOGLE_PICKER_APP_ID` with the Google Cloud project number expected by Picker. Do not document or commit production key values.

OAuth consent must include `https://www.googleapis.com/auth/drive.file` in addition to `openid` and `email`; do not add restricted Drive scopes. Existing connected accounts that did not grant `drive.file` must reconnect from Studio before Picker file selection is ready. The refresh token remains encrypted and server-side. The short-lived Picker access token exists only in browser memory for the modal attempt, and all source file and output folder selections are revalidated server-side before Studio persists safe metadata.
