...
| Unified manifest v2 maintenance | Unit-tested / needs E2E validation | Single destination/output folder-based UI action is unit-tested to expose only `Проверить / обновить manifest` and to call the v2 maintenance helper. Coverage includes dry-run read-only behavior, v1→v2 migration reporting, backup on v1 apply, v2 refresh without mixed top-level v1 `entries`, checked_at-only no-op reporting, separated `documents`/`sources`, replaceable `standard_check` observations, source/document linking by `doc_id`/`doc_link`, orphan source preservation, no Google Docs mutation/create-doc calls, no provider/LLM calls, recursive scan guardrails, and compatibility with v2 source skip behavior (`done` + same settings, `imported_done`, fresh/stale `in_progress`). Real Colab Drive backup/write behavior still needs E2E validation. |
