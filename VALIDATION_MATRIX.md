...
| Docs-only existing Google Docs standardization | Unit-tested / needs E2E validation | Destination/output folder-based UI grouping, folder counting, nested-folder hint, visited-folder guard, scan limits, MIME filtering, dry-run/apply branching, strict current-standard detection, old-standard migration, conservative metadata, and no provider/create-doc/manifest helper calls are unit-tested; real Colab Drive/Docs runtime rewrite still needs E2E validation. |
| Existing Google Docs manifest registration | Unit-tested / needs E2E validation | Destination/output folder-based dry-run/apply branching, no Docs mutation, no provider/LLM/create-doc calls, doc_id-based manifest signatures, structured status classification (`current_standard` / `outdated_standard` / `unstructured` / `unreadable`), recursive scan visited-folder guard, scan limits, dry-run with no manifest creation/save, and manifest upsert/no-duplicate behavior are unit-tested; real Colab Drive/Docs manifest writes still need E2E validation. |
| Existing Google Docs manifest registration reconciliation | Unit-tested / needs E2E validation | Source-linked manifest detection by explicit `doc_id` and Google Doc ID parsed from `doc_link` is unit-tested, including exclusion of `existing_google_doc` / `doc_registered` registry entries so source-linked matches remain informational and do not count as `already_registered`; dry-run no manifest creation/save, no Google Docs mutation, and no provider/LLM/create-doc calls are covered by unit tests; real Colab Drive/Docs reporting still needs E2E validation. |
| Manifest v2 migration / standardization | Unit-tested / needs E2E validation | Backup creation before apply writes, dry-run read-only behavior, v1→v2 conversion, source skip compatibility for v2 sources, manifest/document-standard decoupling, replaceable `standard_check` observation updates, source/document linking by `doc_id`/`doc_link`, orphan source preservation, and no Google Docs/provider mutation are covered by unit tests; real Colab Drive backup/write behavior still needs E2E validation. |
