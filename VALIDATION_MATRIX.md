...
| Manual pre-transcription project segmentation | Parser/static/unit-tested / needs runtime validation | Parser validation, mode availability, distinct manifest identity, safe preview, temp cleanup path and provider-contract static checks are covered without provider calls. Manual Colab validation is still required. |
