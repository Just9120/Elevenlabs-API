...
| Existing Docs backfill metadata and Created-at preservation | Unit-tested / needs runtime validation | Unit tests cover temporary ElevenLabs/scribe_v2/Russian/unknown-speakers metadata defaults, visible `Created at` formatting as `YYYY-MM-DD HH:MM UTC`, preservation priority from existing metadata to Drive `createdTime` to `unknown`, no standardization-time fallback, no visible source/standardized-at fields, current v1.2 strict detection, and transcript body preservation; real Google Docs runtime metadata fetch/rewrite still needs validation. |
| Unified manifest v2 maintenance | Unit-tested / needs E2E validation | Single destination/output folder-based UI action is unit-tested to expose only `Проверить / обновить manifest` and to call the v2 maintenance helper. Coverage includes dry-run read-only behavior, selected-folder scan versus global manifest before/after report totals, v1→v2 migration reporting, backup on v1 apply, v2 refresh without mixed top-level v1 `entries`, checked_at-only no-op reporting, separated `documents`/`sources`, replaceable `standard_check` observations, source/document linking by `doc_id`/`doc_link`, orphan source preservation, no Google Docs mutation/create-doc calls, no provider/LLM calls, recursive scan guardrails, and compatibility with v2 source skip behavior (`done` + same settings, `imported_done`, fresh/stale `in_progress`). Real Colab Drive backup/write behavior still needs E2E validation. |
| UX/report clarity for manifest and docs-only standardization | Unit-tested / needs E2E validation | Report text distinguishes selected-folder scan counters from global manifest totals, dry-run preview/write status, no Docs mutation in manifest flow, no provider calls, and docs-only apply impact; Colab runtime readability still needs E2E validation. |
...
| Legacy import/backfill helpers | Unit-tested / needs E2E validation | Legacy source-matching helpers are not the primary current UI flow. Current user-facing Docs maintenance is covered by docs-only standardization and unified manifest v2 maintenance rows above. |
