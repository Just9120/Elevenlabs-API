...
| Manual pre-transcription project segmentation | Parser/static/unit-tested / needs runtime validation | Parser validation, duplicate-label rejection, optional third-field document titles, duplicate requested-title rejection when suffixes are disabled, deterministic numeric suffix allocation when enabled, segmented routing before parent manifest/conflict checks, safe source-check preview, exact audio re-encode splitting, distinct manifest identity, temp cleanup path and provider-contract static checks are covered without provider calls. Manual Colab validation is still required. |
