...
Этот workflow обслуживает глобальный workspace catalog `VoiceOps Workspace/manifest/elevenlabs_transcription_manifest.json`. Он сканирует выбранный destination/output folder, но manifest остаётся глобальным, а не per-folder. После успешной обычной транскрибации manifest v2 теперь сразу синхронизирует запись `sources` с соответствующей записью `documents`; maintenance нужен для последующей сверки/refresh, а не как основной способ появления новых transcription Docs в каталоге.
...
- **Скан выбранной папки** = only the current selected destination/output folder scan.
- **Манифест до изменений** = global manifest totals before preview.
- **Изменения по выбранной папке** = what this selected folder would add/refresh in the global manifest.
- **Манифест после применения, предварительный просмотр** = global manifest totals if apply happened.
...
That is expected when the global manifest already contains records from other folders. **Скан выбранной папки** is the 5 Docs in the folder you selected. **Манифест после применения, предварительный просмотр** is the global manifest if apply happened, so it can show 86 total documents. Successful transcription-created Docs should already enter `v2.documents` immediately; if maintenance still proposes additions, they are reconciliation/backfill candidates rather than the normal runtime catalog path.
