...
The bounded single-worker/small-source ElevenLabs-to-Google-Docs core is production-live with one controlled exactly-one-output canary. This does not prove every selected mode or the newer catalog revision. PR #192 fixed the project-creation navigation race and passed exact-main repository, Studio, Docker-build, and authenticated-browser CI. Its web component is deployed, while the API remains on the compatible pre-`0016` baseline after post-merge CD stopped fail-closed on non-root access to operator-owned file secrets. The active corrective item and the still-separate backup, migration, API rollout, catalog dry-run, and apply gates are tracked in `docs/delivery-plan.md`.
