...
The bounded single-worker/small-source ElevenLabs-to-Google-Docs core is production-live with one controlled exactly-one-output canary. This does not prove every selected mode or the newer catalog revision. PR #192 fixed the project-creation navigation race, and PR #193 merged the protected non-root secret runtime. Exact-main CI is green at `93de8cf`; post-merge API CD built the new image and read production revision `0015_user_source_retention` through the new bootstrap, then stopped before replacement because the image head is `0016_transcript_catalog_entries`. The prior API remains live. A tagged catalog pre-migration PostgreSQL snapshot is now verified; the separately authorized migration, intended API rollout, catalog dry-run, and apply gates are tracked in `docs/delivery-plan.md`.
