...
- **Studio PWA foundation** — `apps/studio/`: installable Russian-first UI foundation for `studio.librechat.online`. PWA-FOUNDATION-01 is UI-only: no provider calls, Google integration, uploads, jobs, queues or persistence.

## Studio PWA

Frontend workspace: `apps/studio/`. Production scaffolding: `deploy/studio/`, `.github/workflows/studio-ci.yml`, `.github/workflows/studio-cd.yml`, `scripts/deploy_studio.sh`, and `docs/runbooks/studio-deploy.md`. The PWA container is designed for host nginx reverse proxy to `127.0.0.1:8181` and does not replace the Colab workflow.
