...
Existing Google Docs manifest registration is a separate docs-only manifest flow. It may read Google Docs only to classify transcript structure, but it must not store transcript text or Google Docs contents in manifest entries, logs, analytics, or reports. Apply mode may store doc IDs, doc names, doc links, paths, MIME type, structured status, timestamps, and classification/source metadata; dry-run must remain the default. It must not mutate Google Docs content, create Docs, or call STT/provider/LLM APIs.
