...
def test_worker_settings_defaults_and_bounds(monkeypatch):
    ...
    assert Settings(realtime_draft_ttl_seconds=3600)
    assert Settings(realtime_draft_ttl_seconds=604800)
    ...
    with pytest.raises(ValidationError): Settings(realtime_draft_ttl_seconds=3599)
    with pytest.raises(ValidationError): Settings(realtime_draft_ttl_seconds=604801)
