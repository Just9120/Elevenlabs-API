...
def test_worker_settings_defaults_and_bounds(monkeypatch):
    ...
    assert (s.source_upload_ttl_seconds, s.source_retention_ttl_seconds)==(3600,86400)
    assert Settings(source_upload_ttl_seconds=900)
    assert Settings(source_upload_ttl_seconds=86400)
    assert Settings(source_retention_ttl_seconds=3600)
    assert Settings(source_retention_ttl_seconds=2592000)
    ...
    with pytest.raises(ValidationError): Settings(source_upload_ttl_seconds=86401)
    with pytest.raises(ValidationError): Settings(source_retention_ttl_seconds=3599)
    with pytest.raises(ValidationError): Settings(source_retention_ttl_seconds=2592001)
    with pytest.raises(ValidationError): Settings(source_presign_ttl_seconds=59)
