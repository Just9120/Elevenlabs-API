...
state="$(cat {str(state_file)!r})"
code="$(cat {str(exit_file)!r})"
image="$(cat {str(image_file)!r})"
if [[ "$1" == compose ]]; then
  shift
  ...
  case "$1" in
    ps)
      all=false
      for arg in "$@"; do [[ "$arg" == "-a" ]] && all=true; done
      if [[ "$state" == absent ]]; then exit 0; fi
      if [[ "$all" == true || "$state" == running || "$state" == restarting ]]; then echo cid; fi
      ;;
    run)
      if [[ "${{@: -1}}" == current ]]; then echo {current_revision!r}; else echo abc; fi
      ;;
    up)
      echo up
      printf running > {str(state_file)!r}
      printf {rollback_image!r} > {str(image_file)!r}
      ;;
    *) exit 7 ;;
  esac
elif [[ "$1" == inspect ]]; then
  ...
  target="${{@: -2:1}}"
  [[ "$*" == *elevenlabs-studio-worker:rollback-candidate* ]] || exit 9
  echo {rollback_head!r}
elif [[ "$1" == stop ]]; then
  printf {stop_state!r} > {str(state_file)!r}
  printf {stop_exit!r} > {str(exit_file)!r}
elif [[ "$1" == start ]]; then
  printf running > {str(state_file)!r}
elif [[ "$1" == tag ]]; then
...
def test_drain_exit_semantics(tmp_path: Path) -> None:
...
def test_status_reports_drain_state_and_identity(tmp_path: Path) -> None:
    cases = [("absent", "0", "drain_state=absent"), ("running", "0", "drain_state=running"), ("exited", "0", "drain_state=gracefully-drained"), ("exited", "137", "drain_state=abnormal-exit"), ("exited", "143", "drain_state=abnormal-exit")]
    for i, (st, code, expected) in enumerate(cases):
        ...
        assert f"container_state={st}" in proc.stdout and expected in proc.stdout
        if st != "absent":
            assert f"exit_code={code}" in proc.stdout
        assert_no_forbidden(calls)
    proc, _ = run_cmd(tmp_path / "match", "status", state="exited", exit_code="0", commit_tag="present", commit_image="sha256:worker-old")
    assert "commit_tag_state=present" in proc.stdout and "identity_match=yes" in proc.stdout
    proc, _ = run_cmd(tmp_path / "absenttag", "status", state="exited", exit_code="0", commit_tag="absent")
    assert "commit_tag_state=absent" in proc.stdout and "identity_match=unknown" in proc.stdout
    proc, _ = run_cmd(tmp_path / "mismatch", "status", state="exited", exit_code="0", commit_tag="present", commit_image="sha256:new")
    assert "identity_match=no" in proc.stdout


def test_resume_only_gracefully_drained_same_image(tmp_path: Path) -> None:
    proc, calls = run_cmd(tmp_path / "ok", "resume", state="exited", exit_code="0")
    assert proc.returncode == 0 and "STUDIO_WORKER_RESUMED" in proc.stdout
...
def test_rollback_schema_order_and_api_tag_isolation(tmp_path: Path) -> None:
    ...
    assert any("compose --env-file deploy/studio/.env -f deploy/studio/compose.platform.yml up -d --no-deps --force-recreate studio-worker" in c for c in calls)
    assert_no_forbidden(calls)
