...
def test_revision_safety_cases(tmp_path: Path) -> None:
    ...
    for name, edit in [("nohead", "revision = '0008_alt'"), ("multi", "down_revision = None")]:
        ...
        f = repo / "apps/studio-api/alembic/versions/0011_diagnostic_debug_sessions.py"
        f.write_text(f.read_text().replace('revision = "0011_diagnostic_debug_sessions"', edit.replace(chr(39), chr(34))), encoding="utf-8")
