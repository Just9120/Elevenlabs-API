...
""")
    ...
    current = state.get("current", "0019_job_media_clip")
...
def test_successful_host_preflight(tmp_path: Path) -> None:
    ...
    assert "repository Alembic head | pass | exactly one repository Alembic head matches expected source head: 0019_job_media_clip" in proc.stdout
    assert "production Alembic revision | pass | exactly one known production database revision was reported: 0019_job_media_clip" in proc.stdout
    assert "revision equality | pass | production database revision 0019_job_media_clip equals repository head 0019_job_media_clip" in proc.stdout
    assert any(
...
def test_revision_safety_cases(tmp_path: Path) -> None:
    ...
    f = repo / "apps/studio-api/alembic/versions/0019_job_media_clip.py"
    f.write_text(f.read_text().replace('revision = "0019_job_media_clip"', 'revision = "0019_wrong_head"'), encoding="utf-8")
...
def test_revision_output_is_known_normalized_metadata_only(tmp_path: Path) -> None:
    ...
    assert "revision equality | blocked | production database revision 0011_diagnostic_debug_sessions does not equal repository head 0019_job_media_clip" in mismatch.stdout
